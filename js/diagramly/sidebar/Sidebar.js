...
(function () {
  ...
  Sidebar.prototype.gcp2 = [
    ...
    "Icons Networking",
    "Icons CI CD",
    "Icons Integration Services",
    "Icons API Management",
    "Icons Internet of Things",
    "Icons Databases",
    "Icons Storage",
    "Icons Security",
    "Icons Migration",
    "Icons Hybrid and Multi Cloud",
    "Icons Open Source Icons",
  ];

  Sidebar.prototype.gcpicons = [
  ...
  Sidebar.prototype.cisco_safe = [
    "Architecture",
    "Business Icons",
    "Capability",
    "Design",
    "IoT Things Icons",
    "People Places Things Icons",
    "Security Icons",
    "Technology Icons",
    "Threat",
  ];

  Sidebar.prototype.sap = [
    "Annotations",
    "Area Shapes",
    "BTP SaaS",
    "Colored Connectors",
  ...
  Sidebar.prototype.configuration = [
    ...
    { id: "threatModeling" },
    { id: "kubernetes" },
    { id: "cisco19", prefix: "cisco19", libs: Sidebar.prototype.cisco19 },
    { id: "citrix2", prefix: "citrix2", libs: ["Authentication", "Cloud Services", "Components", "Locations", "NetScaler", "Networking", "Resources", "Users and Devices"] },
    {
      ...
      libs: [
        ...
        "Management and Governance Service Media",
        "Migrate Service",
  ...
  Sidebar.prototype.addEntry = function (tags, fn) {
    if (this.currentSearchEntryLibrary != null && this.enabledLibraries != null && mxUtils.indexOf(this.enabledLibraries, this.currentSearchEntryLibrary.id) < 0) {
      return fn;
    } else {
  ...
  Sidebar.prototype.refresh = function () {
    ...
    if (ui.scratchpad != null) {
      ui.loadLibrary(new StorageLibrary(ui, ui.scratchpad.getData(), ".scratchpad"));
    }
  };
  ...
  Sidebar.prototype.showPalette = function (id, visible) {
    ...
    if (elts != null) {
      var vis = visible != null ? (visible ? "" : "none") : elts[0].style.display == "none" ? "" : "none";
  ...
  Sidebar.prototype.showEntries = function (entries, remember, force) {
    ...
    for (var i = 0; i < this.configuration.length; i++) {
      ...
      if (this.configuration[i].id != "search") {
        this.showPalettes(this.configuration[i].prefix || "", this.configuration[i].libs || [this.configuration[i].id], visible[this.configuration[i].id] == true);
      }
    }
  ...
  Sidebar.prototype.updateEntries = function () {
    ...
    if (Editor.currentTheme == "simple" || Editor.currentTheme == "sketch" || Editor.currentTheme == "min") {
      stdEntries = [
        { title: mxResources.get("searchShapes"), id: "search" },
    ...
    this.entries = [
      { title: mxResources.get("standard"), entries: stdEntries },
      {
        ...
        entries: [
          { title: "Active Directory", id: "active_directory", image: IMAGE_PATH + "/sidebar-active_directory.png" },
      ...
      },
      {
        ...
        entries: [
          { title: "Alibaba Cloud", id: "alibaba_cloud", image: IMAGE_PATH + "/sidebar-alibaba_cloud.png" },
      ...
      },
      {
        ...
        entries: [
          { title: mxResources.get("cabinets"), id: "cabinets", image: IMAGE_PATH + "/sidebar-cabinets.png" },
  ...
  if (urlParams["savesidebar"] == "1") {
    Sidebar.prototype.addFoldingHandler = function (title, content, funct) {
      ...
      title.style.backgroundImage = content.style.display == "none" ? "url('" + Editor.arrowRightImage + "')" : "url('" + Editor.arrowDownImage + "')";
      ...
      mxEvent.addListener(
        ...
        mxUtils.bind(this, function (evt) {
          if (mxEvent.getSource(evt).nodeName == "BUTTON") {
            ...
            this.editorUi.editor.convertImages(
              root,
              mxUtils.bind(this, function (body) {
  ...
  Sidebar.prototype.openLibraries = function (libs) {
    ...
    for (var i = 0; i < libs.length; i++) {
      ...
      if (config != null) {
        var temp = this.openLibrary(config, libs[i].lib || libs[i].id);
  ...
  Sidebar.prototype.openLibrary = function (config, lib) {
    ...
    if (elts != null && elts.length > 1 && elts[1].firstChild != null && (elts[1].firstChild.firstChild == null || elts[1].firstChild.style.display == "none")) {
      elts[0].click();
    }
          ...
          if (shapeStyle != null) {
            ...
            if (lastDot > 0) {
              ...
              if (tags != null) {
                ...
                var semi = style.indexOf(";");
                style = "shape=" + pkg + "." + stc.toLowerCase() + ";" + (semi < 0 ? "" : style.substring(semi + 1));
                this.setCurrentSearchEntryLibrary(shapes[i].id, shapes[i].lib);
                this.createVertexTemplateEntry(style, parseInt(shapes[i].w), parseInt(shapes[i].h), "", stc.replace(/_/g, " "), null, null, this.filterTags(tags.join(" ")));

                if (this.currentSearchEntryLibrary != null) {
                  this.addLibForStyle(this.getKeyStyle(style), this.currentSearchEntryLibrary);
                }
  ...
  Sidebar.prototype.initPalettes = function () {
    ...
    this.addImagePalette(
      "finance",
      "Clipart / Finance",
      imgDir + "/lib/clip_art/finance/",
      "_128x128.png",
      ["Arrow_Down", "Arrow_Up", "Coins", "Credit_Card", "Dollar", "Graph", "Pie_Chart", "Piggy_Bank", "Safe", "Shopping_Cart", "Stock_Down", "Stock_Up"],
      ["Arrow_Down", "Arrow Up", "Coins", "Credit Card", "Dollar", "Graph", "Pie Chart", "Piggy Bank", "Safe", "Shopping Basket", "Stock Down", "Stock Up"],
    );
    ...
    this.addImagePalette(
      "networking",
      "Clipart / Networking",
      imgDir + "/lib/clip_art/networking/",
      "_128x128.png",
      [
        "Bridge",
        "Certificate",
    ...
    this.addImagePalette("people", "Clipart / People", imgDir + "/lib/clip_art/people/", "_128x128.png", [
      "Suit_Man",
      "Suit_Man_Black",
      "Suit_Man_Blue",
      "Suit_Man_Green",
    ...
    this.addImagePalette(
      ...
      imgDir + "/lib/clip_art/telecommunication/",
      "_128x128.png",
      ["BlackBerry", "Cellphone", "HTC_smartphone", "iPhone", "Palm_Treo", "Signal_tower_off", "Signal_tower_on"],
      ["BlackBerry", "Cellphone", "HTC smartphone", "iPhone", "Palm Treo", "Signaltower off", "Signaltower on"],
    );
    this.setCurrentSearchEntryLibrary();
          ...
          for (var k = 0; k < entry.libs.length; k++) {
            mxUtils.bind(this, function (lib) {
              ...
              if (lib.data == null && lib.url != null && !lib.preload && preloadCount >= this.maxPreloadCount) {
                this.addPalette(
                  entry.id + "." + k,
                  this.editorUi.getResource(lib.title),
                  false,
                  mxUtils.bind(this, function (content, title) {
                ...
                this.addPalette(
                  ...
                  this.editorUi.getResource(lib.title),
                  false,
                  mxUtils.bind(this, function (c, t) {
                    content = c;
                    title = t;
                    barrier();
                  }),
                ...
                if (lib.data != null) {
                  ...
                  this.editorUi.editor.loadUrl(
                    url,
                    mxUtils.bind(this, function (temp) {
                      try {
      ...
      mxStencilRegistry.loadStencilSet(
        ...
        mxUtils.bind(this, function (packageName, stencilName, displayName, w, h) {
          if ((this.createdSearchIndex != null && ignore == null) || mxUtils.indexOf(ignore, stencilName) < 0) {
            var entry = { style: "shape=" + packageName + stencilName + style, w: Math.round(w * scale), h: Math.round(h * scale) };
        ...
        results.push(
          mxUtils.bind(this, function () {
            return this.createVertexTemplate(
              "shape=image;html=1;verticalAlign=top;" + "verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;imageAspect=0;" + "aspect=fixed;image=" + url,
              width,
              height,
              "",
            );
    ...
    if (ICONSEARCH_PATH != null && searchClosedLibraries) {
      success = mxUtils.bind(this, function (results, len, more, terms) {
        if (!this.editorUi.isOffline() && results.length <= count / 4) {
          ...
          mxUtils.get(
            ICONSEARCH_PATH + "?q=" + encodeURIComponent(searchTerms) + "&p=" + pg + "&c=" + count,
            mxUtils.bind(
              this,
              function (req) {
                try {
                  // Ignore response if nothing or error returned
                  if (req.getStatus() >= 200 && req.getStatus() <= 299 && req.getText() != null && req.getText().length > 0) {
                    try {
  ...
  Sidebar.prototype.itemClicked = function (cells, ds, evt) {
    ...
    if (cells != null && graph.getSelectionCount() == 1 && graph.getModel().isVertex(cells[0])) {
      ...
      if (graph.getModel().isEdge(graph.getSelectionCell()) && graph.getModel().getTerminal(graph.getSelectionCell(), false) == null && graph.getModel().isVertex(target)) {
