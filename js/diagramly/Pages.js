...
SelectPage.prototype.execute = function () {
  ...
  if (this.page != null && prevIndex >= 0) {
    ...
    graph.gridEnabled = graph.gridEnabled && (!this.ui.editor.isChromelessView() || urlParams["grid"] == "1");
...
EditorUi.prototype.getImageForPage = function (page, sourcePage, sourceGraph, addFonts) {
  ...
  var svgRoot = graph.getSvg(null, null, null, null, null, null, null, null, null, null, addFonts, null, null, null, true, true);
  ...
  if (!this.editor.graph.standalone) {
    ...
    this.editor.graph.model.addListener(
      ...
      mxUtils.bind(this, function (sender, evt) {
        ...
        if (this.currentPage != null && !(changes.length == 1 && changes[0] instanceof SelectPage)) {
...
EditorUi.prototype.scrollToPage = function (page, force) {
  var index = page != null ? this.getPageIndex(page) : this.getSelectedPageIndex();
  var scrollPage = page != null ? page : this.currentPage;
  ...
  if (
    ...
    this.tabScroller.children[index].scrollIntoView({ block: "nearest", inline: page != null ? "nearest" : "center" });
...
Graph.prototype.saveViewState = function (vs, node, ignoreTransient, resolveReferences, sourcePage) {
  ...
  var pf = vs != null ? vs.pageFormat : typeof mxSettings === "undefined" || this.defaultPageFormat != null ? mxGraph.prototype.pageFormat : mxSettings.getPageFormat();
  ...
  if (vs != null) {
    ...
    var bgImg = this.getBackgroundImageObject(vs.backgroundImage, resolveReferences, sourcePage);
...
Graph.prototype.setViewState = function (state, removeOldExtFonts) {
  if (state != null) {
    ...
    this.pageFormat = typeof mxSettings === "undefined" || this.defaultPageFormat != null ? mxGraph.prototype.pageFormat : mxSettings.getPageFormat();
...
Graph.prototype.setScrollbarPositions = function (state, dx, dy) {
  if (state != null && state.scrollLeft != null && state.scrollTop != null) {
    this.container.scrollLeft = dx * this.view.scale + state.scrollLeft;
    this.container.scrollTop = dy * this.view.scale + state.scrollTop;
  }
};
...
EditorUi.prototype.insertPage = function (page, index, node) {
  if (this.editor.graph.isEnabled()) {
    ...
    page = page != null ? page : this.createPage(null, this.createPageId(), node);
...
EditorUi.prototype.clonePages = function (pages) {
  ...
  for (var i = 0; i < pages.length; i++) {
    try {
      ...
      errors.push(mxResources.get("pageWithNumber", [i + 1]) + ": " + e.toString());
    }
  }
...
EditorUi.prototype.checkTabScrollerOverflow = function () {
  if (this.tabScroller != null && this.tabContainer != null && this.tabContainer.children.length > 2) {
    ...
    if (!overflow) {
      ...
      if (Math.ceil(this.tabScroller.scrollLeft) + this.tabScroller.offsetWidth >= this.tabScroller.scrollWidth) {
...
EditorUi.prototype.isPageInsertTabVisible = function () {
  return urlParams["embed"] == 1 || (this.getCurrentFile() != null && this.getCurrentFile().isEditable());
};
...
EditorUi.prototype.addTabListeners = function (page, tab) {
  ...
  var onMouseUp = mxUtils.bind(this, function (evt) {
    if (!graph.isMouseDown && ((pageWasActive && (mxEvent.isTouchEvent(evt) || mxEvent.getSource(evt) == elt)) || mxEvent.isPopupTrigger(evt))) {
...
EditorUi.prototype.createPageMenu = function (page, label) {
  return mxUtils.bind(this, function (menu, parent) {
    if (urlParams["embed"] != 1) {
      if (!mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp && this.getServiceName() == "draw.io" && !navigator.standalone) {
        menu.addItem(
