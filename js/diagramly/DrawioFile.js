 ...
 */
window.DrawioFile = function (ui, data) {
...
DrawioFile.prototype.synchronizeFile = function (success, error, latestFile) {
  EditorUi.debug("DrawioFile.synchronizeFile", [this], "latestFile", [latestFile], "savingFile", this.savingFile);
  ...
  if (this.savingFile) {
    ...
    if (this.sync != null) {
      this.sync.fileChanged(
        mxUtils.bind(this, function (patched) {
          if (acceptResponse) {
            window.clearTimeout(timeoutThread);
...
DrawioFile.prototype.updateFile = function (success, error, abort, shadow, immediate, latestFile) {
  if (abort == null || !abort()) {
    EditorUi.debug("DrawioFile.updateFile", [this], "immediate", immediate, "invalidChecksum", this.invalidChecksum, "latestFile", [latestFile]);
    ...
    if (this.ui.getCurrentFile() != this || this.invalidChecksum) {
      ...
      var doUpdate = mxUtils.bind(this, function (latestFile) {
        try {
          if (abort == null || !abort()) {
            EditorUi.debug("DrawioFile.updateFile", [this], "invalidChecksum", this.invalidChecksum, "latestFile", [latestFile]);
            ...
            if (this.ui.getCurrentFile() != this || this.invalidChecksum) {
              ...
              if (latestFile != null) {
                this.mergeFile(latestFile, success, error, shadow, immediate);
              } else {
...
DrawioFile.prototype.mergeFile = function (file, success, error, diffShadow, immediate) {
  ...
  try {
    ...
    if (pages != null && pages.length > 0) {
      ...
      var patches = [this.ui.diffPages(diffShadow != null ? diffShadow : shadow, pages)];
      ...
      if (!ignored) {
        try {
          ...
          var changes = !this.isModified() ? null : this.ui.diffPages(shadow, this.isRealtime() ? this.ownPages : this.ui.pages);
          ...
          if (checksum != null && checksum != current) {
            this.checksumError(error, patches, null, null, "mergeFile", checksum, current, file.getCurrentRevisionId());
            ...
            if (this.sync != null) {
              var pending = this.sync.patchRealtime(patches, DrawioFile.LAST_WRITE_WINS ? changes : null, null, immediate);
            ...
            // Patches the current document
            this.patch(patches, DrawioFile.LAST_WRITE_WINS ? changes : null);
          }
          ...
          try {
            if (reportError) {
              ...
              EditorUi.logError("Error in mergeFile", null, this.getMode() + "." + this.getId(), uid, e);
            }
          } catch (e2) {
            // ignore
          }
        ...
        EditorUi.debug("File.mergeFile", [this], "file", [file], "ignored", ignored);
...
DrawioFile.prototype.checksumError = function (fn, patches, details, etag, functionName, checksum, current, rev) {
  ...
  try {
    var user = this.getCurrentUser();
    var uid = user != null ? user.id : "unknown";
    var id = this.getId() != "" ? this.getId() : "(" + this.ui.hashValue(this.getTitle()) + ")";
    ...
    this.getLatestVersion(
      mxUtils.bind(this, function (latestFile) {
        ...
        try {
          ...
          try {
            var node = latestFile.initialData != null && latestFile.initialData.length > 0 ? mxUtils.parseXml(latestFile.initialData).documentElement : null;
          ...
          EditorUi.logError(
            "Checksum " + type + " in " + functionName,
            null,
            this.getMode() + "." + id,
            "user_" +
              uid +
              (this.sync != null ? "-client_" + this.sync.clientId : "-nosync") +
              "-bytes_" +
              bytes +
              "-patches_" +
              patches.length +
              ...
              "-latestRev_" +
              this.ui.hashValue(latestFile.getCurrentRevisionId()) +
          ...
          EditorUi.logEvent({
            category: "CHECKSUM-ERROR-SYNC-FILE-" + id,
            action: functionName,
            label: "user_" + uid + (this.sync != null ? "-client_" + this.sync.clientId : "-nosync") + "-bytes_" + bytes + "-patches_" + patches.length + "-size_" + this.getSize(),
          });
        } catch (e) {
          // ignore
        }
...
DrawioFile.prototype.sendErrorReport = function (title, details, error, max) {
  try {
    var shadow = this.compressReportData(this.getAnonymizedXmlForPages(this.getShadowPages()), 25000);
    var data = this.compressReportData(this.getAnonymizedXmlForPages(this.ui.pages), 25000);
    var user = this.getCurrentUser();
    ...
    EditorUi.sendReport(
      title +
        ...
        cid +
        "\nPrefix=" +
        this.ui.editor.graph.model.prefix +
        "\nSync=" +
        DrawioFile.SYNC +
        (this.sync != null ? (this.sync.enabled ? " enabled" : "") + (this.sync.isConnected() ? " connected" : "") : "") +
        "\nPlugins=" +
        (mxSettings.settings != null ? mxSettings.getPlugins() : "null") +
        "\n\nStats:\n" +
...
DrawioFile.prototype.reloadFile = function (success, error) {
  try {
    ...
    var changes = this.ui.diffPages(this.getShadowPages(), this.isRealtime() ? this.ownPages : this.ui.pages);
...
DrawioFile.prototype.copyFile = function (success, error) {
  this.ui.editor.editAsNew(this.ui.getFileData(true), this.ui.getCopyFilename(this));
};
  ...
  if (patches != null) {
    ...
    try {
      if (undoable) {
        ...
        var pages = this.ui.applyPatches(this.ui.pages, patches, true, resolver, this.isModified());

        for (var i = 0; i < pages.length; i++) {
          var index = mxUtils.indexOf(this.ui.pages, pages[i]);

          if (index < 0) {
            this.ui.insertPage(pages[i], Math.min(i, this.ui.pages.length));
          } else {
        ...
        this.ui.pages = this.ui.applyPatches(this.ui.pages, patches, true, resolver, this.isModified());
      }
    ...
    EditorUi.debug("DrawioFile.patch", [this], "patches", patches, "resolver", resolver, "undoable", undoable);
  }
...
DrawioFile.prototype.createData = function () {
  ...
  if (this.isRealtime()) {
    ...
    if (this.ui.currentPage != null) {
      var ownPage = this.ui.getPageById(this.ui.currentPage.getId(), this.ownPages);
  ...
  var result = this.ui.getFileData(null, null, null, null, null, null, null, null, this, !this.isCompressed());
...
DrawioFile.prototype.setLocked = function (locked) {
  this.ui.fileNode.setAttribute("locked", locked ? "true" : "false");
...
DrawioFile.prototype.share = function () {
  if (this.ui.drive != null) {
    this.ui.confirm(
      mxResources.get("saveItToGoogleDriveToCollaborate", [this.getTitle()]),
      mxUtils.bind(this, function () {
        this.ui.pickFolder(
          App.MODE_GOOGLE,
          mxUtils.bind(this, function (folderId) {
            ...
            this.ui.createFile(
              this.getTitle(),
              this.ui.getFileData(null, null, null, null, null, null, null, null, this),
              null,
              App.MODE_GOOGLE,
              null,
              true,
      ...
      null,
      mxResources.get("saveToGoogleDrive", null, "Save to Google Drive"),
...
DrawioFile.prototype.setData = function (data) {
  ...
  EditorUi.debug("DrawioFile.setData", [this], "data", [data]);
};
...
DrawioFile.prototype.getRealtimeState = function () {
  return this.sync != null && this.sync.p2pCollab != null ? this.sync.p2pCollab.getState() : 3 /* CLOSED */;
};
...
DrawioFile.prototype.getRealtimeError = function () {
  return this.sync != null && this.sync.p2pCollab != null ? this.sync.p2pCollab.getLastError() : null;
};
...
DrawioFile.prototype.startSync = function () {
  if (
    (DrawioFile.SYNC == "auto" || DrawioFile.SYNC == "fast") &&
    urlParams["stealth"] != "1" &&
    (urlParams["rt"] == "1" || !this.ui.editor.chromeless || this.ui.editor.editable)
  ) {
    if (this.sync == null) {
...
DrawioFile.prototype.addAllSavedStatus = function (status) {
  if (this.ui.statusContainer != null && this.ui.getCurrentFile() == this) {
    ...
    var rev = this.isRevisionHistorySupported() && status != mxUtils.htmlEntities(mxResources.get(this.savingStatusKey)) + "..." ? 'data-action="revisionHistory" ' : "";
    this.ui.editor.setStatus(
      "<div " +
        ...
        '">' +
        status +
        (this.isLocked() ? ' <img class="geToolbarButton" data-action="properties" ' + 'style="margin-left:4px;flex-shrink:0;" src="' + Editor.lockedImage + '"/>' : "") +
        "</div>",
    );
  }
};
...
DrawioFile.prototype.saveDraft = function (data) {
  try {
    ...
    this.ui.setDatabaseItem(".draft_" + this.draftId, JSON.stringify(draft));

    EditorUi.debug("DrawioFile.saveDraft", [this], "draftId", this.draftId, [draft]);
  } catch (e) {
...
DrawioFile.prototype.removeDraft = function () {
  try {
    if (this.draftId != null) {
      EditorUi.debug("DrawioFile.removeDraft", [this], "draftId", this.draftId);
    ...
    if (err instanceof Error && err.message != null && err.message != "") {
      ...
      this.ui.updateStatus(
        mxUtils.bind(this, function () {
          ...
          var status = mxUtils.htmlEntities(mxResources.get("unsavedChangesClickHereToSave")) + (msg != null && msg != "" ? " (" + mxUtils.htmlEntities(msg) + ")" : "");
          var action = 'data-action="' + (this.ui.mode == null || !this.isEditable() ? "saveAs" : "save") + '"';
          this.ui.editor.setStatus("<div " + action + ' title="' + status + '" class="geStatusAlert">' + status + "</div>");
        }),
      );
      ...
      if (EditorUi.enableDrafts && (this.getMode() == null || EditorUi.isElectronApp)) {
        ...
        this.saveDraftThread = window.setTimeout(
          mxUtils.bind(this, function () {
            this.lastDraftSave = Date.now();
            this.saveDraftThread = null;
            this.saveDraft();
...
DrawioFile.prototype.addConflictStatus = function (message, fn) {
  this.ui.updateStatus(
    mxUtils.bind(this, function () {
      ...
      this.setConflictStatus(mxUtils.htmlEntities(mxResources.get("fileChangedSync")) + (message != null && message != "" ? " (" + mxUtils.htmlEntities(message) + ")" : ""), fn);
    }),
  );
...
DrawioFile.prototype.setConflictStatus = function (message, fn) {
  this.ui.editor.setStatus(
    '<div title="' +
      message +
      '" ' +
      (fn != null ? 'data-action="statusFunction"' : "") +
      ' class="geStatusAlert">' +
      message +
      '<img data-link="https://www.drawio.com/doc/faq/synchronize" src="' +
      Editor.helpImage +
      '" style="margin-left:2px;cursor:help;"/></div>',
    fn,
  );
};
...
DrawioFile.prototype.showRefreshDialog = function (success, error, message) {
  ...
  if (this.ui.editor.isChromelessView() && !this.ui.editor.editable) {
    this.ui.alert(
      mxResources.get("fileChangedSync"),
      mxUtils.bind(this, function () {
        this.reloadFile(success, error);
      }),
    );
    ...
    this.ui.showError(
      mxResources.get("warning") + " (" + message + ")",
...
DrawioFile.prototype.showCopyDialog = function (success, error, overwrite) {
  ...
  this.ui.showError(
    mxResources.get("externalChanges"),
...
DrawioFile.prototype.redirectToNewApp = function (error, details) {
  ...
  if (!this.redirectDialogShowing) {
    ...
    var url =
      window.location.protocol + "//" + window.location.host + "/" + this.ui.getSearch(["create", "title", "mode", "url", "drive", "splash", "state"]) + "#" + this.getHash();
...
DrawioFile.prototype.handleFileSuccess = function (saved) {
  ...
  if (this.ui.getCurrentFile() == this) {
    EditorUi.debug(
      ...
      this.isModified(),
      "remoteFileChanged",
      this.sync == null ? "n/a" : this.sync.remoteFileChanged,
    );
    ...
    if (this.isModified()) {
      ...
      if (this.isTrashed()) {
        this.ui.updateStatus(
          mxUtils.bind(this, function () {
            this.addAllSavedStatus(mxUtils.htmlEntities(mxResources.get(this.allChangesSavedKey)) + " (" + mxUtils.htmlEntities(mxResources.get("fileMovedToTrash")) + ")");
          }),
        );
      } else {
        this.ui.updateStatus(
      ...
      if (manual) {
        ...
        this.ui.updateStatus(
          mxUtils.bind(this, function () {
            ...
            this.ui.editor.setStatus(
              '<div class="geStatusAlert">' + mxUtils.htmlEntities(mxResources.get("error")) + (msg != null ? " (" + mxUtils.htmlEntities(msg) + ")" : "") + "</div>",
            );
          }),
        );
      }
    ...
    if (this.ui.spinner.spin(document.body, mxResources.get("updatingDocument"))) {
      this.synchronizeFile(
        mxUtils.bind(this, function () {
          ...
          if (this.ui.spinner.spin(document.body, mxResources.get(this.savingSpinnerKey))) {
            var isRepoFile = this.constructor == GitHubFile || this.constructor == GitLabFile;
            this.save(true, success, error, null, null, isRepoFile && err != null ? err.commitMessage : null);
          }
        }),
        error,
      );
...
DrawioFile.prototype.compressionChanged = function (compressed) {
  ...
  if (pages != null) {
    for (var i = 0; i < pages.length; i++) {
      ...
      if (pageNode != null && (this.ui.currentPage == null || this.ui.currentPage.getId() != pages[i].getId())) {
        ...
        if (compressed) {
          if (xml.length == 0 && modelNode != null) {
            ...
            EditorUi.debug("DrawioFile.compressionChanged", [this], "Page " + i + " compressed");
          }
        } else {
          if (xml.length > 0 && modelNode == null) {
            EditorUi.removeChildNodes(pageNode);
            ...
            EditorUi.debug("DrawioFile.compressionChanged", [this], "Page " + i + " decompressed");
          }
        }
      }
    }
...
DrawioFile.prototype.fileChanged = function (sync) {
  ...
  EditorUi.debug("DrawioFile.fileChanged", [this], "autosaveDelay", this.autosaveDelay, "autosave", this.isAutosave(), "saving", this.savingFile);
  ...
  if (this.isAutosave()) {
    ...
    if ((!this.isAutosaveOptional() || !this.ui.editor.autosave) && !this.inConflictState) {
      this.addUnsavedStatus();
    }
  }
...
DrawioFile.prototype.createSecret = function (success) {
  ...
  if (Editor.enableRealtimeCache && this.sync != null && !this.isOptimisticSync()) {
    this.sync.createToken(
      secret,
      mxUtils.bind(this, function (token) {
        EditorUi.debug("DrawioFile.createSecret", [this], "secret", secret, "token", token);
  ...
  try {
    ...
    try {
      if (this.sync == null || this.isOptimisticSync()) {
        ...
        this.sync.fileSaved(pages, lastDesc, success, error, token, checksum);
      }
    } catch (e) {
      this.inConflictState = true;
      this.invalidChecksum = true;
      ...
      try {
        ...
        EditorUi.logError("Error in fileSaved", null, this.getMode() + "." + this.getId(), uid, e);
      } catch (e2) {
        // ignore
      }
    }

    EditorUi.debug("DrawioFile.fileSaved", [this], "savedData", [savedData], "desc", [lastDesc], "inConflictState", this.inConflictState, "invalidChecksum", this.invalidChecksum);
  } catch (e) {
    this.descriptorChanged();
  ...
  var thread = window.setTimeout(
    mxUtils.bind(this, function () {
      try {
        ...
        EditorUi.debug("DrawioFile.autosave", [this], "thread", thread, "modified", this.isModified(), "now", this.isAutosaveNow(), "saving", this.savingFile);
