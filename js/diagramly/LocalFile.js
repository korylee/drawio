...
/**
 */
window.LocalFile = function (ui, data, title, temp, fileHandle, desc, editable) {
...
LocalFile.prototype.isEditable = function () {
  return DrawioFile.prototype.isEditable.apply(this, arguments) && (this.editable == null || this.editable);
};
    ...
    if (this.fileHandle != null) {
          ...
          mxUtils.bind(this, function (writable) {
            this.fileHandle.getFile().then(
              mxUtils.bind(this, function (newDesc) {
                ...
                EditorUi.debug("LocalFile.saveFile", [this], "desc", [this.desc], "newDesc", [newDesc], "conflict", this.desc.lastModified != newDesc.lastModified);

                if (overwrite || this.desc.lastModified == newDesc.lastModified) {
                  writable.write(binary ? this.ui.base64ToBlob(data, "image/png") : data).then(
      ...
      if (this.ui.isOfflineApp() || this.ui.isLocalFileSave()) {
        ...
        if (data.length < MAX_REQUEST_SIZE) {
          ...
          new mxXmlRequest(SAVE_URL, "format=" + format + "&xml=" + encodeURIComponent(data) + "&filename=" + encodeURIComponent(title) + (binary ? "&binary=1" : "")).simulate(
            document,
            "_blank",
          );
        } else {
