 ...
 * @constructor
 */
window.StorageFile = function (ui, data, title) {
...
StorageFile.insertFile = function (ui, title, data, success, error, file) {
  StorageFile.doInsertFile(file != null ? file : new StorageFile(ui, data, title), success, error);
};
...
StorageFile.prototype.saveFile = function (title, revision, success, error, retry) {
            ...
            this.getLatestVersion(
              mxUtils.bind(this, function (file) {
                if (file.getDescriptor() != this.getDescriptor()) {
                  EditorUi.debug("StorageFile.saveFile", [this], "conflict", [file]);

                  this.mergeFile(
                    file,
                    mxUtils.bind(this, function () {
                      if (retry >= this.maxRetries || this.invalidChecksum || this.inConflictState) {
                        ...
                        this.retrySave(
                          mxUtils.bind(this, function () {
                            this.updateFileData();
                            this.saveFile(title, revision, success, error, retry + 1);
                          }),
                        );
                      }
                    }),
  ...
  EditorUi.debug("StorageFile.saveFile", [this], "title", title, "revision", revision, "retry", retry);
};
...
StorageFile.listLocalStorageFiles = function (type) {
  ...
  for (var i = 0; i < localStorage.length; i++) {
    ...
    if (key.length > 0 && key.charAt(0) != "." && value.length > 0) {
      var isFile = (type == null || type == "F") && (value.substring(0, 8) === "<mxfile " || value.substring(0, 5) === "<?xml" || value.substring(0, 12) === "<!--[if IE]>");
