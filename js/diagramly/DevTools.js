    ...
    Graph.prototype.getTooltipForCell = function (cell) {
      ...
      if (geo != null) {
        ...
        if (state != null && state.absolutePoints != null) {
          ...
          tip += "x/y=" + parseFloat(geo.x) + "," + parseFloat(geo.y) + "<br>" + "w/h=" + parseFloat(geo.width) + "x" + parseFloat(geo.height);

          if (state != null) {
            tip += "<br>pos=" + parseFloat(state.x) + "," + parseFloat(state.y) + "<br>" + "size=" + parseFloat(state.width) + "x" + parseFloat(state.height);
          }
        }

        if (cell.style != null) {
          tip +=
            '<br>style=<div style="display:inline-block;vertical-align:bottom;white-space:nowrap;width:480px;' +
            'overflow:hidden;text-overflow:ellipsis;">' +
            mxUtils.htmlEntities(cell.style) +
            "</span>";
        }
      }
