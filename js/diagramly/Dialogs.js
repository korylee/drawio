...
var StorageDialog = function (editorUi, fn, rowLimit) {
  ...
  if (mxClient.IS_SVG && isLocalStorage && urlParams["gapi"] != "0" && (document.documentMode == null || document.documentMode >= 10)) {
...
var EmbedDialog = function (editorUi, result, timeout, ignoreSize, previewFn, title, tweet, previewTitle, filename) {
  ...
  if (EmbedDialog.showPreviewOption && !mxIsElectron && !navigator.standalone && validUrl) {
          ...
          if (doc != null) {
            doc.writeln(
              "<html><head><title>" +
                ...
                (result.substring(0, 7) == "<iframe" ? result : mxUtils.htmlEntities(result)) +
                "</body></html>",
            );
            doc.close();
          } else {
...
var CreateGraphDialog = function (editorUi, title, type) {
  ...
  this.init = function () {
    ...
    var okBtn = mxUtils.button(mxResources.get("insert"), function (evt) {
      ...
      var pt = mxEvent.isAltDown(evt) ? editorUi.editor.graph.getFreeInsertPoint() : editorUi.editor.graph.getCenterInsertPoint(graph.getBoundingBoxFromGeometry(cells, true));
...
var BackgroundImageDialog = function (editorUi, applyFn, img, color, showColor) {
  ...
  applyBtn = mxUtils.button(mxResources.get("apply"), function () {
    ...
    urlChanged(null, function (url) {
      applyFn(
        url != "" && url != null ? new mxImage(url, widthInput.value, heightInput.value) : null,
        url == null,
        newBackgroundColor,
...
var ParseDialog = function (editorUi, title, defaultType) {
  ...
  function parse(text, type, evt) {
    if (type == "plantUmlPng" || type == "plantUmlSvg" || type == "plantUmlTxt") {
      if (editorUi.spinner.spin(document.body, mxResources.get("inserting"))) {
        var graph = editorUi.editor.graph;
        var format = type == "plantUmlTxt" ? "txt" : type == "plantUmlPng" ? "png" : "svg";
            ...
            cell =
              format == "txt"
                ...
                : graph.insertVertex(
                    ...
                    w,
                    h,
                    "shape=image;noLabel=1;verticalAlign=top;aspect=fixed;imageAspect=0;" + "image=" + editorUi.convertDataUri(data) + ";",
                  );
            graph.setAttributeForCell(cell, "plantUmlData", JSON.stringify({ data: text, format: format }, null, 2));
          } finally {
            graph.getModel().endUpdate();
          }
      ...
      if (editorUi.spinner.spin(document.body, mxResources.get("inserting"))) {
        editorUi.parseMermaidDiagram(
          ...
          mxUtils.bind(this, function (xml) {
            editorUi.spinner.stop();
            var graph = editorUi.editor.graph;
            graph.setSelectionCells(editorUi.importXml(xml, Math.max(insertPoint.x, 20), Math.max(insertPoint.y, 20), true, null, null, true));
        ...
        if (tmp.substring(0, 12).toLowerCase() == "create table") {
          ...
          var rowCell = new mxCell(
            ...
            "shape=tableRow;horizontal=0;startSize=0;swimlaneHead=0;swimlaneBody=0;fillColor=none;" +
              ...
              "strokeColor=inherit;top=0;left=0;right=0;bottom=" +
              (pk ? "1" : "0") +
              ";",
          );
          rowCell.vertex = true;
          ...
          var left = new mxCell(
            ...
            "shape=partialRectangle;overflow=hidden;connectable=0;fillColor=none;" + "strokeColor=inherit;top=0;left=0;bottom=0;right=0;" + (pk ? "fontStyle=1;" : ""),
          );
          left.vertex = true;
          rowCell.insert(left);
      ...
      if (cells.length > 0) {
        ...
        try {
          ...
          if (type == "horizontalFlow" || type == "verticalFlow") {
            var flowLayout = new mxHierarchicalLayout(graph, type == "horizontalFlow" ? mxConstants.DIRECTION_WEST : mxConstants.DIRECTION_NORTH);
  ...
  if (defaultType == "formatSql" || (defaultType == "mermaid" && editorUi.getServiceName() != "draw.io" && editorUi.getServiceName() != "atlassian")) {
  ...
  if (EditorUi.enablePlantUml && Graph.fileSupport && !editorUi.isOffline() && defaultType == "plantUml") {
  ...
  if (defaultType == "plantUml" && window.PLANT_URL == "https://plant-aws.diagrams.net") {
    ...
    warning.innerHTML =
      '<a href="https://github.com/jgraph/drawio/discussions/5074" target="_blank" ' +
      'rel="noopener noreferrer" style="color: #c00; text-decoration: underline;">PlantUML is deprecated</a>';
    buttons.appendChild(warning);
  }

  if ((!editorUi.isOffline() || mxClient.IS_CHROMEAPP) && (defaultType == "mermaid" || defaultType == "plantUml")) {
    buttons.appendChild(editorUi.createHelpIcon(defaultType == "mermaid" ? "https://mermaid.js.org/intro/" : "https://plantuml.com/"));
  }
...
var NewDialog = function (
  ...
  rightHighlightBorder = rightHighlightBorder != null ? rightHighlightBorder : "2px dashed light-dark(#29b6f2, #00a8ff)";
  ...
  if (showName) {
    mxUtils.write(
      ...
      (smallScreen
        ...
        : editorUi.mode == null || editorUi.mode == App.MODE_GOOGLE || editorUi.mode == App.MODE_BROWSER
  ...
  function showTooltip(xml, x, y, elt, title, url) {
    ...
    if (xml != null && mxUtils.isAncestorNode(document.body, elt)) {
      ...
      editorUi.sidebar.createTooltip(
        ...
        true,
        function () {
          wasVisible = editorUi.sidebar.tooltip != null && editorUi.sidebar.tooltip.style.display != "none";
  ...
  function create() {
    if (selectedElt == generateElt && templateXml == null && generateButton != null && generateInput != null) {
      if (callback && editorUi.spinner.spin(document.body, mxResources.get("generate") + " '" + generateInput.value + "'")) {
        ...
        if (title != null && title.length > 0) {
          function doSave(mode, folderId, filename) {
            editorUi.createFile(
              filename,
              templateXml,
              templateLibs != null && templateLibs.length > 0 ? templateLibs : null,
              mode,
              function () {
                if (!insertWasPressed) {
                  editorUi.hideDialog();
              ...
              folderId,
              null,
              templateClibs != null && templateClibs.length > 0 ? templateClibs : null,
            );
          }
          ...
          if (editorUi.mode == App.MODE_GOOGLE || editorUi.mode == App.MODE_ONEDRIVE) {
            ...
            editorUi.pickFolder(
              ...
              editorUi.mode != App.MODE_GOOGLE || editorUi.stateArg == null || editorUi.stateArg.folderId == null,
            );
          }
        }
      }
  ...
  tmplSearchInput.style.cssText = "width:105px;height:16px;border:1px solid #d3d3d3;" + "padding: 3px 20px 3px 3px;font-size: 12px;border-radius:0px;";
  ...
  var mouseDownHandler = function (evt) {
    generatePreviewWasVisible = editorUi.sidebar.tooltip != null && editorUi.sidebar.tooltip.style.display != "none";
  };

  var mouseUpHandler = function (evt) {
    if (!generatePreviewWasVisible && lastAiXml != null) {
      var previewXml = "<mxfile><diagram>" + Graph.compress(lastAiXml) + "</diagram></mxfile>";
      showTooltip(previewXml, mxEvent.getClientX(evt), mxEvent.getClientY(evt), generateElt, lastAiTitle);
    }
  };
      ...
      editorUi.generateOpenAiMermaidDiagram(
        ...
        function (xml) {
          ...
          if (selectedElt == generateElt && generateForm.style.display == "none") {
            generateBackground = "url(" + Editor.createSvgDataUri(mxUtils.getXml(editorUi.getSvgForXml(xml))) + ")";
    ...
    function loadXmlData(url, callback) {
      if (xmlData == null) {
        ...
        if (/^https?:\/\//.test(realUrl)) {
          realUrl = editorUi.editor.isCorsEnabledForUrl(realUrl) ? realUrl : PROXY_URL + "?url=" + encodeURIComponent(realUrl);
        } else {
      ...
      if (url != null && !loading && editorUi.sidebar.currentElt != elt) {
          ...
          var xml =
            ...
            Graph.compress(
              '<mxGraphModel><root><mxCell id="0"/><mxCell id="1" parent="0"/>' +
                '<mxCell id="2" value="" style="shape=image;image=' +
                extImg.src +
                ';imageAspect=1;" parent="1" vertex="1">' +
                '<mxGeometry width="' +
                extImg.naturalWidth +
                '" height="' +
                extImg.naturalHeight +
                '" as="geometry" />' +
        ...
        loadXmlData(url, function (xml) {
          if (loading && editorUi.sidebar.currentElt == elt) {
            try {
              showTooltip(xml, mxEvent.getClientX(evt), mxEvent.getClientY(evt), elt, tooltipTitle);
            } catch (e) {
  ...
  if (Editor.enableAi && editorUi.isExternalDataComms() && editorUi.getServiceName() == "draw.io" && typeof mxMermaidToDrawio !== "undefined" && window.isMermaidEnabled) {
  ...
  function filterTemplates() {
    ...
    var temp = mxResources.get(results.length == 0 ? "noResultsFor" : "resultsFor", [searchTerms]);
  ...
  function initUi() {
    ...
    function getEntryTitle(cat, templateList) {
      var label = mxResources.get(cat, null, cat.substring(0, 1).toUpperCase() + cat.substring(1));

      if (label.length > 18) {
        label = label.substring(0, 18) + "&hellip;";
      }

      return label + (templateList != null ? " (" + templateList.length + ")" : "");
    }
    ...
    for (var cat in categories) {
      ...
      if (categories[cat].content != null) {
        ...
        entry.style.cssText =
          "display:block;cursor:pointer;padding:6px;white-space:nowrap;margin-bottom:-1px;overflow:hidden;text-overflow:ellipsis;user-select:none;transition: all 0.5s;";
            ...
            if (typeof node.getAttribute !== "undefined") {
              if (node.nodeName == "parsererror") {
                if (window.console != null) {
                  console.log("Parser error in " + templateFile + ": " + node.textContent);
                }
                ...
                if (url != null) {
                  ...
                  var tempObj = {
                    ...
                    tooltip: node.getAttribute("name") || node.getAttribute("url"),
  ...
  if (!compact && urlParams["embed"] != "1" && !createOnly && !mxClient.IS_ANDROID && !mxClient.IS_IOS && urlParams["noDevice"] != "1") {
        ...
        function (fileUrl) {
          if (fileUrl != null && fileUrl.length > 0) {
            editorUi.editor.loadUrl(
              editorUi.editor.getProxiedUrl(fileUrl),
              function (data) {
                templateXml = data;
                templateLibs = null;
 ...
 *
 */
var SaveDialog = function (editorUi, title, saveFn, disabledModes, data, mimeType, base64Encoded, defaultMode, folderPickerMode, enabledModes, saveBtnLabel) {
  var div = document.createElement("div");
  ...
  if (data != null && mimeType != null && mimeType.substring(0, 6) == "image/" && (mimeType.substring(0, 9) != "image/svg" || mxClient.IS_SVG)) {
    ...
    if ((mimeType == "image/png" || mimeType == "image/svg+xml") && typeof window.ClipboardItem === "function" && navigator.clipboard != null) {
      copyBtn = mxUtils.button(
        mxResources.get("copy"),
        function () {
          editorUi.writeImageToClipboard(
            dataUri,
            null,
            null,
            mimeType,
    ...
    if (Editor.popupsAllowed && (disabledModes == null || mxUtils.indexOf(disabledModes, "_blank") < 0)) {
  ...
  if (editorUi.editor.diagramFileTypes != null && mimeType == null && folderPickerMode == null) {
    left = left.cloneNode(false);
    right = right.cloneNode(false);

    mxUtils.write(left, mxResources.get("type") + ":");

    typeSelect = FilenameDialog.createFileTypes(editorUi, saveAsInput, editorUi.editor.diagramFileTypes);
  ...
  function addStorageEntry(mode, path, id, selected, title, entryType) {
    ...
    if (
      ...
      if (isLocal || editorUi.getServiceForName(mode) != null) {
        ...
        if (!isLocal && editorUi.isOffline(mode == App.MODE_GOOGLE && urlParams["gapi-stealth"] == "1")) {
          option.setAttribute("disabled", "disabled");
          state = " (" + mxResources.get("offline") + ")";
        }

        if (entryType == "pick") {
          option.innerHTML = mxUtils.htmlEntities(title) + dash + mxUtils.htmlEntities(mxResources.get("pickFolder")) + "..." + state;
          option.setAttribute("value", "pickFolder-" + mode);
          option.setAttribute("title", title + " - " + mxResources.get("pickFolder") + "...");
        } else {
          ...
          option.innerHTML = mxUtils.htmlEntities(title) + (shortPath != null ? dash + mxUtils.htmlEntities(shortPath) : "") + state;
          option.setAttribute("title", title + (path != null ? " (" + path + ")" : "") + (id != null && decodeURIComponent(id) != path ? " [" + id + "]" : ""));
          option.setAttribute("value", entryId);
          entries[entryId] = { option: option, mode: mode, path: path, id: id };

          if (SaveDialog.lastValue == entryId && defaultMode == null && !option.getAttribute("disabled") == "disabled") {
            selected = true;
          } else if (selected == null) {
            if (entryType == "root") {
              selected = (defaultMode == null && editorUi.mode == mode) || (mode != null && mode == defaultMode);
    ...
    editorUi.pickFolder(
      ...
      function (result) {
        ...
        if (mode == App.MODE_GOOGLE && result.docs != null && result.docs.length > 0) {
          ...
          entry = { mode: mode, path: result.value[0].name, id: OneDriveFile.prototype.getIdOf(result.value[0]) };
        } else if ((mode == App.MODE_GITHUB || mode == App.MODE_GITLAB) && result != null && result.length > 0) {
  ...
  function checkExtension() {
    if (
      typeSelect != null &&
      entries[storageSelect.value] != null &&
      ...
      if (ext != null && title.indexOf(".") < 0 && name.indexOf(".") < 0) {
  ...
  function addStorageEntries() {
    ...
    var allowDevice = !Editor.useLocalStorage || urlParams["storage"] == "device" || (editorUi.getCurrentFile() != null && urlParams["noDevice"] != "1");
    ...
    if (EditorUi.nativeFileSupport && allowDevice) {
      addStorageEntry(
        ...
        null,
        null,
        editorUi.mode == App.MODE_DEVICE || (disabledModes != null && mxUtils.indexOf(disabledModes, App.MODE_BROWSER) >= 0) ? true : null,
      );
    }
  ...
  function storageChanged() {
    ...
    mxUtils.write(
      saveBtn,
      saveBtnLabel != null ? saveBtnLabel : mxResources.get(storageSelect.value == "download" || storageSelect.value == "_blank" || folderPickerMode != null ? "ok" : "save"),
    );
  ...
  if (!editorUi.isOffline() || mxClient.IS_CHROMEAPP) {
    btns.appendChild(editorUi.createHelpIcon("https://www.drawio.com/doc/faq/save-file-formats"));
  }
...
var CreateDialog = function (
  ...
  // Disables SVG preview if SVG is not supported in browser
  if (urlParams["noDevice"] != "1" && data != null && mimeType != null && mimeType.substring(0, 6) == "image/" && (mimeType.substring(0, 9) != "image/svg" || mxClient.IS_SVG)) {
    ...
    if (!mxClient.IS_FF && mimeType == "image/png" && navigator.clipboard != null && typeof window.ClipboardItem === "function") {
      copyBtn = mxUtils.button(mxResources.get("copy"), function (evt) {
        ...
        var cbi = new ClipboardItem({ "image/png": blob, "text/html": new Blob([html], { type: "text/html" }) });
        navigator.clipboard
          .write([cbi])
          .then(
            mxUtils.bind(this, function () {
  ...
  if (!Editor.useLocalStorage || urlParams["storage"] == "device" || (editorUi.getCurrentFile() != null /* && !mxClient.IS_IOS*/ && urlParams["noDevice"] != "1")) {
...
var LinkDialog = function (editorUi, initialValue, btnLabel, fn, showPages, showNewWindowOption, linkTarget) {
  ...
  var mainBtn = mxUtils.button(btnLabel, function () {
    editorUi.hideDialog();
    var value = pageRadio.checked ? (pageSelect.value !== "pageNotFound" ? pageSelect.value : initialValue) : linkInput.value;
  ...
  LinkDialog.filePicked = function (data) {
    if (data.action == google.picker.Action.PICKED) {
      ...
      if (data.docs[0].mimeType == "application/mxe" || (data.docs[0].mimeType != null && data.docs[0].mimeType.substring(0, 23) == "application/vnd.jgraph.")) {
    ...
    addButton("github", function () {
      editorUi.gitHub.pickFile(function (path) {
        if (path != null) {
          ...
          linkInput.value = "https://github.com/" + org + "/" + repo + "/blob/" + ref + "/" + path;
          linkInput.focus();
        }
      });
    });
    ...
    addButton("gitlab", function () {
      editorUi.gitLab.pickFile(function (path) {
        if (path != null) {
          ...
          linkInput.value = DRAWIO_GITLAB_URL + "/" + org + "/" + repo + "/blob/" + ref + "/" + path;
          linkInput.focus();
        }
      });
    });
...
var RevisionDialog = function (editorUi, revs, restoreFn) {
  ...
  ((errorNode.style.position = "absolute"), (errorNode.style.display = "none"));
  ...
  mxEvent.addGestureListeners(
    ...
    function (e) {
      ...
      try {
        var curr = diagrams[currentPage] != null ? currentDiagrams[diagrams[currentPage].getAttribute("id")] : null;
        ...
        if (curr == null) {
          ...
          cmpGraph.view.scaleAndTranslate(graph.view.scale, graph.view.translate.x, graph.view.translate.y);
    ...
    mxUtils.bind(this, function (e) {
      if (currentDoc != null) {
        ...
        var dlg = new TextareaDialog(
              ...
              mxEvent.addListener(patchSelect, "change", function (evt) {
                ...
                if (patchSelect.value != "allPages") {
                  ...
                  if (patch[EditorUi.DIFF_UPDATE][pageId] != null) {
                    ...
                    pagePatch[EditorUi.DIFF_UPDATE][pageId] = patch[EditorUi.DIFF_UPDATE][pageId];
                  }
                }

                input.value = JSON.stringify(pagePatch == null ? patch : pagePatch, null, 2);
              });
          ...
          true,
          null,
          mxResources.get(editorUi.editor.graph.isEnabled() ? "merge" : "close"),
        );
        ...
        if (ts.getTime() >= 0) {
          ...
          if (item == revs[revs.length - 1]) {
            ...
            if (ts.toDateString() === today) {
              ...
              mxUtils.write(date, ts.toLocaleDateString() + " " + ts.toLocaleTimeString());
            }
          }
          ...
          function updateGraph(xml) {
            ...
            if (node != null) {
              ...
              if (node.nodeName == "mxfile") {
                ...
                if (diagrams.length > 1) {
                  ...
                  for (var i = 0; i < diagrams.length; i++) {
                    var pageOption = document.createElement("option");
                    pageOption.setAttribute("title", name + " (" + diagrams[i].getAttribute("id") + ")");
                    pageOption.setAttribute("value", i);
                    var name = diagrams[i].getAttribute("name") || mxResources.get("pageWithNumber", [i + 1]);
              ...
              fileInfo.innerText = "";
              mxUtils.write(fileInfo, (shortUser != null ? shortUser + " " : "") + ts.toLocaleDateString() + " " + ts.toLocaleTimeString());
  ...
  } else if (file == null || (editorUi.drive == null && file.constructor == window.DriveFile) || (editorUi.dropbox == null && file.constructor == window.DropboxFile)) {
    container.style.display = "none";
...
var DraftDialog = function (editorUi, title, xml, editFn, discardFn, editLabel, discardLabel, ignoreFn, drafts) {
    ...
    for (var i = 0; i < drafts.length; i++) {
      ...
      mxUtils.write(
        ...
        ts0.toLocaleDateString() +
          ...
          " - " +
          (ts0.toDateString() != ts1.toDateString() || true ? ts1.toLocaleDateString() : " ") +
          " " +
  ...
  var tb = document.createElement("div");
  tb.className = "geToolbarContainer";
  tb.style.cssText = "box-shadow:none !important;background-color:transparent;" + "padding:2px;border-style:none !important;bottom:30px;";
  ...
  this.init = function () {
    ...
    if (node.nodeName == "mxfile") {
      ...
      if (diagrams.length > 1) {
        ...
        for (var i = 0; i < diagrams.length; i++) {
          var pageOption = document.createElement("option");
          mxUtils.write(pageOption, diagrams[i].getAttribute("name") || mxResources.get("pageWithNumber", [i + 1]));
          pageOption.setAttribute("value", i);
...
var FindWindow = function (ui, x, y, w, h, withReplace) {
  ...
  function search(internalCall, trySameCell, stayOnPage) {
    ...
    if (searchStr.length > 0) {
      ...
      for (i = 0; i < cells.length; i++) {
        ...
        if (state != null && state.cell.value != null && (active || firstMatch == null) && (graph.model.isVertex(state.cell) || graph.model.isEdge(state.cell))) {
  ...
  var btn = mxUtils.button(mxResources.get("find"), function () {
    try {
      searchInput.style.backgroundColor = search() ? "" : "light-dark(#ffcfcf, #ff0000)";
    } catch (e) {
      ui.handleError(e);
    }
  });
    ...
    var replaceFindBtn = mxUtils.button(mxResources.get("replFind"), function () {
      try {
        if (lblMatch != null && lastFound != null) {
          ...
          searchInput.style.backgroundColor = search(false, true) ? "" : "light-dark(#ffcfcf, #ff0000)";
        }
      } catch (e) {
        ui.handleError(e);
      }
...
var FreehandWindow = function (editorUi, x, y, w, h, withBrush) {
  ...
  if (withBrush) {
    ...
    var btn = mxUtils.button(
      ...
      mxUtils.bind(this, function (evt) {
        editorUi.pickColor(
          graph.freehand.getStrokeColor(true),
          function (newColor) {
...
var AdaptiveColorsWindow = function (editorUi, x, y, w, h) {
  ...
  var btn = mxUtils.button(
    mxResources.get("removeIt", [mxResources.get("userDefined")]),
    mxUtils.bind(this, function () {
      editorUi.removeUserDefinedDarkColors(graph.isSelectionEmpty() ? graph.getVerticesAndEdges() : graph.getSelectionCells(), labelCheckbox.checked, backgroundCheckbox.checked);
    }),
  );
...
var ChatWindow = function (editorUi, x, y, w, h) {
  ...
  if (typeof mxMermaidToDrawio !== "undefined" && window.isMermaidEnabled && mxUtils.indexOf(Editor.aiActions, "createPublic") >= 0) {
    createPublicOption.setAttribute("value", "createPublic");
    mxUtils.write(createPublicOption, mxResources.get("create") + " (" + mxResources.get("draw.io") + ")");
  ...
  if (typeof mxMermaidToDrawio !== "undefined" && window.isMermaidEnabled && mxUtils.indexOf(Editor.aiActions, "create") >= 0) {
  ...
  for (var i = 0; i < Editor.aiModels.length; i++) {
    ...
    if (Editor.aiConfigs[model.config] && Editor.aiGlobals[Editor.aiConfigs[model.config].apiKey] != null) {
  ...
  var updateDropdowns = function () {
    inp.setAttribute("placeholder", mxResources.get(typeSelect.value == "create" || typeSelect.value == "createPublic" ? "describeYourDiagram" : "askMeAnything"));
  };
  ...
  function typeChanged() {
    ...
    modelSelect.style.display = typeSelect.value == "createPublic" ? "none" : "";
  }
  ...
  function addMessage(prompt) {
    ...
    if (type != "createPublic" && (aiModel == null || Editor.aiConfigs[aiModel.config] == null)) {
    ...
    if (type == "includeCopyOfMyDiagram" || type == "selectionOnly") {
      ...
      if (type == "selectionOnly") {
        enc.isObjectIgnored = function (obj) {
          return obj.constructor == mxCell && !graph.model.isRoot(obj) && !graph.model.isLayer(obj) && !graph.isCellSelected(obj) && !graph.isAncestorSelected(obj);
        };
      }
    ...
    var processMessage = mxUtils.bind(this, function () {
      ...
      var handleResponse = mxUtils.bind(this, function (data, prompt) {
        var dt = Date.now() - t0;
        EditorUi.debug("EditorUi.ChatWindow.handleResponse", "data", data, "prompt", [prompt], "time", dt);
        var cells = null;
        ...
        if (cells != null && cells.length > 0) {
          ...
          var clickFn = mxUtils.bind(this, function (e) {
            ...
            try {
              if (sentModel != null && page != null && editorUi.getPageIndex(page) != null) {
                ...
                var patch = editorUi.diffCells(sentModel.root, receivedModel.root);
                editorUi.patchPage(page, patch, null, true);
                EditorUi.debug("EditorUi.ChatWindow.handleResponse", "sentModel", sentModel, "receivedModel", receivedModel, "patch", patch);
              } else {
                var pt = graph.getFreeInsertPoint();
                graph.setSelectionCells(graph.importCells(cells, pt.x, pt.y));
                EditorUi.debug("EditorUi.ChatWindow.handleResponse", "cells", graph.getSelectionCell());
              }
            } finally {
          ...
          if (data[1].length > 0) {
            ...
            editorUi.sidebar.createItem(cells, prompt, true, true, bbox.width, bbox.height, true, true, clickFn, null, null, null, null, null, item);
            ...
            if (editorUi.getServiceName() == "draw.io") {
              ...
              btn.setAttribute("title", mxResources.get(!editorUi.isStandaloneApp() ? "openInNewWindow" : "export"));
              buttons.appendChild(btn);
            ...
            if (xml != null && page != null && editorUi.getPageIndex(page) != null) {
        ...
        bubble.scrollIntoView({ behavior: "smooth", block: "end", inline: "nearest" });
      });
          ...
          mxUtils.bind(this, function (timeout) {
            ...
            req.setRequestHeaders = mxUtils.bind(this, function (request, params) {
              ...
              for (var key in config.requestHeaders) {
                request.setRequestHeader(key, Editor.replacePlaceholders(config.requestHeaders[key], resolver));
              }
            });

            EditorUi.debug("EditorUi.ChatWindow.addMessage send", "url", url, "params", params, "aiModel", aiModel, "config", config);
                ...
                if (timeout.clear()) {
                  try {
                    if (req.getStatus() >= 200 && req.getStatus() <= 299) {
                      ...
                      EditorUi.debug("EditorUi.ChatWindow.addMessage response", "params", params, "response", response, "text", [text], "mermaid", [mermaid]);
  ...
  this.window = new mxWindow(mxResources.get("generate"), div, x, y, w, h, true, true);
...
var TagsWindow = function (editorUi, x, y, w, h) {
  ...
  var tagsComponent = editorUi.editor.graph.createTagsDialog(
    mxUtils.bind(this, function () {
      return this.window.isVisible();
    }),
    null,
    function (allTags, updateFn) {
      if (graph.isEnabled()) {
        var dlg = new FilenameDialog(
          editorUi,
          "",
          ...
          null,
          null,
          "https://www.drawio.com/blog/tags-in-diagrams",
        );
...
var MoreShapesDialog = function (editorUi, expanded, entries) {
  ...
  if (editorUi.sidebar.customEntries != null) {
    for (var i = 0; i < editorUi.sidebar.customEntries.length; i++) {
      ...
      for (var j = 0; section.entries != null && j < section.entries.length; j++) {
        ...
        tmp.entries.push({ id: entry.id, title: editorUi.getResource(entry.title), desc: editorUi.getResource(entry.desc), image: entry.preview });
      }
  ...
  if (expanded) {
    ...
    var applyBtn = mxUtils.button(mxResources.get("apply"), function () {
      ...
      if ((Editor.currentTheme == "simple" || Editor.currentTheme == "sketch" || Editor.currentTheme == "min") && Editor.isSettingsEnabled()) {
...
var CropImageDialog = function (editorUi, image, clipPath, fn) {
  ...
  function calcClipPath() {
    ...
    return (
      ...
      mxUtils.format(top) +
      "% " +
      mxUtils.format(right) +
      "% " +
      mxUtils.format(bottom) +
      "% " +
      mxUtils.format(left) +
      "%" +
...
var LibraryDialog = function (editorUi, name, library, initialImages, file, mode, allowBrowser) {
      ...
      if (mimeType == null || mimeType.substring(0, 6) == "image/") {
        if ((data == null && img != null) || entries[data] == null) {
          ...
          if (w > editorUi.maxImageSize || h > editorUi.maxImageSize) {
            var s = Math.min(1, Math.min(editorUi.maxImageSize / Math.max(1, w)), editorUi.maxImageSize / Math.max(1, h));
            w *= s;
            h *= s;
          }
          ...
          if (data != null) {
            ...
            var cells = editorUi.stringToCells(img.xml.charAt(0) == "<" ? img.xml : Graph.decompress(img.xml));

            if (cells.length > 0) {
              editorUi.sidebar.createThumb(cells, ew, eh, wrapper, null, true, false, null, null, graph.shapeBackgroundColor);
          ...
          (function (wrapperDiv, dataParam, imgParam) {
            mxEvent.addListener(rem, "click", function (evt) {
              ...
              for (var i = 0; i < images.length; i++) {
                if ((images[i].data != null && images[i].data == dataParam) || (images[i].xml != null && imgParam != null && images[i].xml == imgParam.xml)) {
                  images.splice(i, 1);
                  break;
                }
              }
          ...
          function updateLabel() {
            ...
            label.style.textOverflow = "ellipsis";
            mxUtils.write(label, entry.title != null && entry.title.length > 0 ? entry.title : mxResources.get("untitled"));
          ...
          var startEditing = function (evt) {
            ...
            if (!mxClient.IS_IOS && !mxClient.IS_FF && (document.documentMode == null || document.documentMode > 9)) {
              if (label.getAttribute("contentEditable") != "true") {
                if (stopEditing != null) {
                  stopEditing();
                  stopEditing = null;
    ...
    return function (data, mimeType, x, y, w, h, img, doneFn, file) {
      if (file != null && EditorUi.isVisioFilename(file.name)) {
        editorUi.importVisio(
          file,
          mxUtils.bind(this, function (xml) {
            addButton(xml, mimeType, x, y, w, h, img, "fixed", mxEvent.isAltDown(evt) ? null : img.substring(0, img.lastIndexOf(".")).replace(/_/g, " "));
          }),
        );
  ...
  function dropHandler(evt) {
    ...
    if (dragSourceIndex != null) {
      if (dropTargetIndex != null && dropTargetIndex < div.children.length) {
        images.splice(dropTargetIndex > dragSourceIndex ? dropTargetIndex - 1 : dropTargetIndex, 0, images.splice(dragSourceIndex, 1)[0]);
      ...
      if (/(\.jpg)($|\?)/i.test(uri) || /(\.png)($|\?)/i.test(uri) || /(\.gif)($|\?)/i.test(uri) || /(\.svg)($|\?)/i.test(uri)) {
  ...
  var btn = mxUtils.button(mxResources.get("export"), function () {
    ...
    if (editorUi.isLocalFileSave()) {
      ...
      new mxXmlRequest(SAVE_URL, "filename=" + encodeURIComponent(filename) + "&format=xml&xml=" + encodeURIComponent(data)).simulate(document, "_blank");
    }
  });
...
var EditShapeDialog = function (editorUi, cell, title) {
  ...
  if (!editorUi.isOffline()) {
    btns.appendChild(editorUi.createHelpIcon("https://www.drawio.com/doc/faq/shape-complex-create-edit"));
  }
...
var FilePropertiesDialog = function (editorUi, publicLink) {
  ...
  } else if (!/(\.html)$/i.test(filename) && !/(\.svg)$/i.test(filename)) {
  ...
  if (file != null && editorUi.getServiceName() == "draw.io" && file.getSize() > 0 && urlParams["embed"] != "1") {
    row = document.createElement("tr");
    td = document.createElement("td");
    td.style.whiteSpace = "nowrap";
    td.style.overflow = "hidden";
  ...
  if (file != null && file.fileObject != null && file.fileObject.path != null) {
    row = document.createElement("tr");
    td = document.createElement("td");
    td.style.whiteSpace = "nowrap";
    td.style.overflow = "hidden";
...
var ConnectionPointsDialog = function (editorUi, cell) {
  ...
  this.init = function () {
    ...
    function createCPoint(x, y, constObj) {
      var cPointStyle =
        "shape=image;points=[];rotatable=0;resizable=0;connectable=0;editable=0;image=" + editorUi.convertDataUri(mxConstraintHandler.prototype.pointImage.src) + ";";
    ...
    var mainCell = new mxCell(cell.value, new mxGeometry(0, 0, geo.width, geo.height), cell.style + ";rotatable=0;resizable=0;connectable=0;editable=0;movable=0;opacity=50;");
    ...
    var gridBtn = editorUi.createToolbarButton(Editor.thinGridImage, mxResources.get("grid"), function (evt) {
      if (mxEvent.isShiftDown(evt)) {
        changeGridSize();
      } else {
    ...
    var deleteBtn = editorUi.createToolbarButton(Editor.trashImage, mxResources.get("delete"), removeCPoints);
    ...
    var applyBtn = mxUtils.button(mxResources.get("apply"), function () {
      ...
      for (var i = 0; i < constraints.length; i++) {
        if (
          i > 0 &&
          constraints[i].x == constraints[i - 1].x &&
          constraints[i].y == constraints[i - 1].y &&
          constraints[i].dx == constraints[i - 1].dx &&
          constraints[i].dy == constraints[i - 1].dy
        ) {
    ...
    if (!editorUi.isOffline()) {
      buttons.appendChild(editorUi.createHelpIcon("https://www.drawio.com/doc/faq/shape-connection-points-customise"));
    }
