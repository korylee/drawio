 ...
 */
export const Menus = (window.Menus = function (editorUi) {
  ...
  this.put(
    ...
    new Menu(
      mxUtils.bind(this, function (menu) {
        ...
        Format.processMenuIcon(this.editorUi.menus.edgeStyleChange(menu, "", keys, [null, null, null, null, null], null, null, true, Format.connectionImage.src)).setAttribute(
          "title",
          mxResources.get("line"),
        );
        Format.processMenuIcon(this.editorUi.menus.edgeStyleChange(menu, "", keys, ["link", null, null, null, null], null, null, true, Format.linkEdgeImage.src)).setAttribute(
          "title",
          mxResources.get("link"),
        );
        Format.processMenuIcon(this.editorUi.menus.edgeStyleChange(menu, "", keys, ["flexArrow", null, null, null, null], null, null, true, Format.arrowImage.src)).setAttribute(
          "title",
          mxResources.get("arrow"),
        );
        Format.processMenuIcon(this.editorUi.menus.edgeStyleChange(menu, "", keys, ["arrow", null, null, null, null], null, null, true, Format.simpleArrowImage.src)).setAttribute(
          "title",
          mxResources.get("simpleArrow"),
        );
        Format.processMenuIcon(
          this.editorUi.menus.edgeStyleChange(menu, "", keys, ["filledEdge", null, null, null, null], null, null, true, Format.filledEdgeImage.src),
        ).setAttribute("title", "Filled Edge");
        Format.processMenuIcon(this.editorUi.menus.edgeStyleChange(menu, "", keys, ["pipe", null, null, null, null], null, null, true, Format.pipeEdgeImage.src)).setAttribute(
          "title",
          "Pipe",
        );
        Format.processMenuIcon(this.editorUi.menus.edgeStyleChange(menu, "", keys, ["wire", null, null, "1", null], null, null, true, Format.wireEdgeImage.src)).setAttribute(
          "title",
          "Wire",
        );
      }),
        ...
        var addItem = mxUtils.bind(this, function (fontFamily) {
          var tr = this.styleChange(
            ...
            function () {
              ...
              ui.fireEvent(new mxEventObject("styleChanged", "keys", [mxConstants.STYLE_FONTFAMILY], "values", [fontFamily], "cells", [graph.cellEditor.getEditingCell()]));
            },
            function () {
      ...
      mxUtils.bind(this, function (menu, parent) {
        ...
        var setFontSize = mxUtils.bind(this, function (fontSize) {
          if (graph.cellEditor.textarea != null) {
            ...
            ui.fireEvent(new mxEventObject("styleChanged", "keys", [mxConstants.STYLE_FONTSIZE], "values", [fontSize], "cells", [graph.cellEditor.getEditingCell()]));
          }
        });
    ...
    new Menu(
      mxUtils.bind(this, function (menu, parent) {
        menu.addItem(
          ...
          null,
          function () {
            graph.distributeCells(true, null, true);
          },
          parent,
        );
        menu.addItem(
          ...
          null,
          function () {
            graph.distributeCells(false, null, true);
          },
          parent,
        );
      }),
  ...
  this.put(
    ...
    new Menu(
      mxUtils.bind(this, function (menu, parent) {
        this.addMenuItems(
          menu,
          (this.editorUi.format != null ? ["format"] : []).concat(
            [
              "outline",
              "layers",
              "-",
...
Menus.prototype.addSubmenu = function (name, menu, parent, label) {
  ...
  if (entry != null) {
    ...
    if (menu.showDisabled || enabled) {
      submenu = menu.addItem(label || mxResources.get(name), null, null, parent, null, enabled);
...
Menus.prototype.addInsertTableCellItem = function (menu, parent) {
  ...
  var isTable = graph.isTable(cell) || graph.isTableRow(cell) || graph.isTableCell(cell);
  ...
  if (isStack) {
    var style = graph.isStack(cell) ? style : graph.getCellStyle(graph.model.getParent(cell));
      ...
      mxUtils.bind(this, function (evt, rows, cols, title, container) {
        var table =
          container || mxEvent.isControlDown(evt) || mxEvent.isMetaDown(evt)
            ? graph.createCrossFunctionalSwimlane(rows, cols, null, null, title || mxEvent.isShiftDown(evt) ? "Cross-Functional Flowchart" : null)
            : graph.createTable(rows, cols, null, null, title || mxEvent.isShiftDown(evt) ? "Table" : null);
        var pt = mxEvent.isAltDown(evt) ? graph.getFreeInsertPoint() : graph.getCenterInsertPoint(graph.getBoundingBoxFromGeometry([table], true));
        var select = null;
  ...
  mxEvent.addGestureListeners(
    ...
    mxUtils.bind(this, function (e) {
      ...
      if (td != null && row2 != null && selected) {
        insertFn(e, row2.sectionRowIndex + 1, td.cellIndex + 1, titleOption.checked, containerOption.checked);
    ...
    menu.addItem(
      ...
      mxUtils.bind(this, function () {
        ...
        try {
          ...
          this.editorUi.fireEvent(new mxEventObject("styleChanged", "cells", edges, "keys", keys, "values", values, "force", cells.length == 0));
        } finally {
          graph.getModel().endUpdate();
        }
      }),
  ...
  this.editorUi.tryAndHandle(
    mxUtils.bind(this, function () {
      ...
      if (cmd != null && graph.cellEditor.isContentEditing()) {
              ...
              if (graph.cellEditor.textarea != null) {
                ...
                this.editorUi.fireEvent(
                  new mxEventObject(
                    "styleChanged",
                    "keys",
                    [cmd == "forecolor" ? mxConstants.STYLE_FONTCOLOR : mxConstants.STYLE_LABEL_BACKGROUNDCOLOR],
                    "values",
                    [color],
                    "cells",
        ...
        this.editorUi.pickColor(color, ColorDialog.createApplyFunction(this.editorUi, key), defaultColor, defaultColorValue, singleColorMode);
      }
    }),
  );
};
...
Menus.prototype.addPopupDeleteItem = function (menu, cell, evt) {
  ...
  if (item != null && item.firstChild != null && item.firstChild.nextSibling != null) {
    item.firstChild.nextSibling.style.color = "red";
    var graph = this.editorUi.editor.graph;

    if (graph.getSelectionCount() > 1) {
      item.firstChild.nextSibling.innerHTML = mxUtils.htmlEntities(mxResources.get("delete") + " (" + graph.getSelectionCount() + ")");
    }
  }
};
...
Menus.prototype.addPopupMenuEditItems = function (menu, cell, evt) {
  if (this.editorUi.editor.graph.isSelectionEmpty()) {
    ...
    this.addMenuItems(menu, ["cut", "copy", "duplicate", "-", "delete", "lockUnlock"], null, evt);
  }
};
...
Menus.prototype.addPopupMenuArrangeItems = function (menu, cell, evt) {
  ...
  if (
    ...
    (graph.getSelectionCount() > 0 && !graph.getModel().isEdge(cell) && !graph.isSwimlane(cell) && graph.getModel().getChildCount(cell) > 0 && graph.isCellEditable(cell))
  ) {
...
Menus.prototype.addPopupMenuCellItems = function (menu, cell, evt) {
  ...
  if (state != null) {
    ...
    if (this.isShowStyleItems() && ss.vertices.length == 0 && ss.edges.length > 0) {
      ...
      if (mxUtils.getValue(state.style, mxConstants.STYLE_EDGE, null) != "entityRelationEdgeStyle" && mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null) != "arrow") {
        ...
        if (graph.getSelectionCount() == 1) {
          ...
          if (hasWaypoints && handler instanceof mxEdgeHandler && handler.bends != null && handler.bends.length > 2) {
            ...
            if (
              ...
              (handler.bends[index] == null || handler.bends[index].node == null || handler.bends[index].node.style.opacity == "")
            ) {
    ...
    if (
      this.isShowCellEditItems() &&
      ((ss.vertices.length == 0 && ss.edges.length > 0) || hasWaypoints || (graph.getModel().isVertex(cell) && graph.getModel().getEdgeCount(cell) > 0))
    ) {
      this.addMenuItems(menu, ["clearWaypoints"], null, evt);
    }

    if (this.isShowCellEditItems() && graph.getSelectionCount() == 1 && graph.isCellEditable(cell)) {
 ...
 */
export const Menubar = (window.Menubar = function (editorUi, container) {
  this.editorUi = editorUi;
 ...
 */
export const Menu = (window.Menu = function (funct, enabled) {
