...
window.EditorUi = function (editor, container, lightbox) {
  ...
  if (!graph.standalone) {
    ...
    if (this.container == document.body && (!this.editor.chromeless || this.editor.editable)) {
    ...
    graph.cellEditor.isStopEditingEvent = function (evt) {
      return (
        ...
        (evt.keyCode == 13 && ((!mxClient.IS_SF && mxEvent.isControlDown(evt)) || (mxClient.IS_MAC && mxEvent.isMetaDown(evt)) || (mxClient.IS_SF && mxEvent.isShiftDown(evt))))
      );
    };
      ...
      if (this.toolbar != null) {
        ...
        if (graph.isEnabled() && ss.edges.length > 0) {
          if (this.toolbar.edgeStyleMenu != null) {
            this.toolbar.edgeStyleMenu.style.backgroundImage = "url(" + this.getImageForEdgeStyle(ss.style) + ")";
          }

          if (this.toolbar.edgeShapeMenu != null) {
            this.toolbar.edgeShapeMenu.style.backgroundImage = "url(" + this.getImageForEdgeShape(ss.style) + ")";
          }
        } else {
          if (this.toolbar.edgeStyleMenu != null) {
 ...
 */
EditorUi.prototype.defaultSidebarWidth = Math.min(screen.width / 2, urlParams["sidebar-entries"] != "large" ? 220 : 230);
...
EditorUi.prototype.createSelectionState = function () {
  ...
  if (Editor.enableCustomProperties) {
    ...
    for (var i = 0; i < vertices.length; i++) {
      this.findCommonProperties(vertices[i], result.customProperties, i == 0, result);
    }

    for (var i = 0; i < edges.length; i++) {
      this.findCommonProperties(edges[i], result.customProperties, vertices.length == 0 && i == 0, result);
    }
  }
...
EditorUi.prototype.findCommonProperties = function (cell, properties, addAll, sstate) {
  ...
  if (state != null && state.shape != null) {
    ...
    if (!state.shape.commonCustomPropAdded) {
      ...
      if (state.shape != null && state.shape.stencil != null && state.shape.stencil.desc != null) {
        ...
        var getStencilColors = mxUtils.bind(this, function (nodeName) {
          ...
          for (var i = 0; i < nodes.length; i++) {
            ...
            if (!mxUtils.isValidColor(name) && !handledKeys[name] && name != "fill" && name != "stroke" && name != "font") {
              handledKeys[name] = true;
              var label = nodes[i].getAttribute("name");
              label = label != null ? label : Editor.getLabelForStylename(name);
              var defaultValue = nodes[i].getAttribute("default");
              var defaultColor = stencil.getDefaultColorValue(nodes[i], graph);
              ...
              props.push({
                name: name,
                type: "color",
                primary: nodes[i].getAttribute("primary") != "false",
                defVal: defaultValue,
                defaultColor: defaultColor,
                undefinedColor: undefinedValue,
                dispName: label,
              });
            }
          }
        ...
        Array.prototype.push.apply(state.shape.customProperties, getStencilColors("fillcolor"));
        Array.prototype.push.apply(state.shape.customProperties, getStencilColors("strokecolor"));
        Array.prototype.push.apply(state.shape.customProperties, getStencilColors("fontcolor"));
      }
...
EditorUi.prototype.createTimeout = function (timeout, fn, error) {
  ...
  var timeoutThread = window.setTimeout(handleError, timeout != null ? timeout : this.timeout);
  ...
  try {
    for (var i = 0; i < cells.length; i++) {
      if (graph.model.isEdge(cells[i]) || graph.model.isVertex(cells[i])) {
        ...
        if (includeLabels && graph.isHtmlLabel(cells[i]) && mxUtils.getValue(style, "html", "0") != "0") {
          tempDiv.innerHTML = Graph.sanitizeHtml(graph.getLabel(cells[i]));

          if (
            Graph.addLightDarkColors(tempDiv, null, null, function (elt, key, value) {
              if (mxUtils.isLightDarkColor(value)) {
                var cssColor = mxUtils.getLightDarkColor(value);
 ...
 * Creates a temporary graph instance for rendering off-screen content.
 */
EditorUi.prototype.createShapePicker = function (x, y, source, callback, direction, afterClick, cells, hovering, getInsertLocationFn, showEdges, startEditing) {
  ...
  getInsertLocationFn =
    getInsertLocationFn != null
      ...
      : function (cells) {
          ...
          return new mxPoint(graph.snap(Math.round(x / graph.view.scale) - graph.view.translate.x - w), graph.snap(Math.round(y / graph.view.scale) - graph.view.translate.y - h));
        };
  ...
  if (cells != null && cells.length > 0) {
    ...
    var addCell = mxUtils.bind(this, function (cell) {
      ...
      if (style != null && urlParams["sketch"] != "1") {
        ...
        this.sidebar.graph.pasteCellStyles([cell], graph.currentVertexStyle, graph.currentEdgeStyle);
      }
      ...
      if (geo != null) {
        var temp = this.sidebar.createVertexTemplateFromCells(
          [cell],
          geo.width,
          geo.height,
          "",
...
EditorUi.prototype.getCellsForShapePicker = function (cell, hovering, showEdges) {
  ...
  if (cell == null) {
    cell = createVertex(graph.appendFontSize(Editor.defaultTextStyle, graph.vertexFontSize), 60, 30, "Text");
  }
  ...
  var cells = [
    ...
    createVertex("shape=waypoint;sketch=0;size=6;pointerEvents=1;points=[];fillColor=none;resizable=0;" + "rotatable=0;perimeter=centerPerimeter;snapToPoint=1;", 20, 20),
  ];
...
EditorUi.prototype.hideShapePicker = function (cancel) {
  if (this.shapePicker != null) {
    ...
    if (this.hoverIcons != null && this.hoverIcons.shapePickerHoverDiv != null) {
      this.hoverIcons.shapePickerHoverDiv = null;
    }
...
EditorUi.prototype.getImageForMarker = function (marker, fill, shape) {
  ...
  if (shape == "flexArrow") {
    ...
    result = fill != "1" ? Format.classicMarkerImage.src : Format.classicFilledMarkerImage.src;
  } else if (marker == mxConstants.ARROW_CLASSIC_THIN) {
    result = fill != "1" ? Format.classicThinMarkerImage.src : Format.openThinFilledMarkerImage.src;
    ...
    result = fill != "1" ? Format.blockMarkerImage.src : Format.blockFilledMarkerImage.src;
  } else if (marker == mxConstants.ARROW_BLOCK_THIN) {
    result = fill != "1" ? Format.blockThinMarkerImage.src : Format.blockThinFilledMarkerImage.src;
  } else if (marker == mxConstants.ARROW_OVAL) {
    result = fill != "1" ? Format.ovalMarkerImage.src : Format.ovalFilledMarkerImage.src;
    ...
    result = fill != "1" ? Format.asyncMarkerImage.src : Format.asyncFilledMarkerImage.src;
  } else if (marker == "dash") {
...
EditorUi.prototype.getImageForEdgeStyle = function (style) {
  ...
  if (es == "orthogonalEdgeStyle" && mxUtils.getValue(style, mxConstants.STYLE_CURVED, null) == "1") {
    ...
    result = mxUtils.getValue(style, mxConstants.STYLE_ELBOW, null) == "vertical" ? Format.verticalElbowImage.src : Format.horizontalElbowImage.src;
  } else if (es == "isometricEdgeStyle") {
    result = mxUtils.getValue(style, mxConstants.STYLE_ELBOW, null) == "vertical" ? Format.verticalIsometricImage.src : Format.horizontalIsometricImage.src;
  }
...
EditorUi.prototype.updatePasteActionStates = function () {
  ...
  paste.setEnabled(
    this.editor.graph.cellEditor.isContentEditing() ||
      ((navigator.clipboard != null || !mxClipboard.isEmpty()) && graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())),
  );
        ...
        addButton(
          mxUtils.bind(this, function (evt) {
            if (this.editor.editButtonFunc != null) {
              ...
              this.editor.editAsNew(this.getEditBlankXml(), null, null, pageId);
            } else {
    ...
    graph.view.validate = function () {
      if (graph.container != null && mxUtils.hasScrollbars(graph.container)) {
        ...
        if (ui.currentPage != null && lastPage != ui.currentPage) {
          ...
          var bbox = graph.getBoundingBoxFromGeometry(graph.model.getCells(), true, null, true);
        ...
        if (!graph.isMouseDown || forcedZoom) {
          updateZoomTimeout = window.setTimeout(
            mxUtils.bind(this, function () {
              ...
              graph.zoom(graph.cumulativeZoomFactor, null, graph.isFastZoomEnabled() ? mult : null);
              var s = graph.view.scale;
    ...
    window.setTimeout(
      mxUtils.bind(
        ...
        function () {
          ...
          this.hsplitPosition = Math.min(this.container.clientWidth - this.hsplit.clientWidth - this.formatWidth, this.hsplitPosition);
          this.refresh();
  ...
  if (!this.editor.extendCanvas) {
    ...
    if (mxUtils.hasScrollbars(c)) {
      if (graph.pageVisible) {
        ...
        c.scrollLeft = Math.floor(Math.min(pad.x, (c.scrollWidth - c.clientWidth) / 2));
 ...
 */
window.ChangeGridColor = function ChangeGridColor(ui, color) {
  this.ui = ui;
  this.color = color;
}
 ...
 * Change types
 */
window.ChangePageSetup = function ChangePageSetup(ui, color, image, format, pageScale) {
  this.ui = ui;
  this.color = color;
...
EditorUi.prototype.setGridColor = function (value, darkMode) {
  ...
  graph.view.gridColor = "light-dark(" + graph.view.defaultGridColor + ", " + graph.view.defaultDarkGridColor + ")";
  graph.view.validateBackground();
...
EditorUi.prototype.updateActionStates = function () {
  ...
  this.actions.get("group").setEnabled(ss.cells.length > 1 || (ss.vertices.length == 1 && graph.model.getChildCount(ss.cells[0]) == 0 && !graph.isContainer(ss.vertices[0])));
  this.actions
    .get("ungroup")
    .setEnabled(!ss.row && !ss.cell && !ss.table && ss.vertices.length > 0 && (graph.isContainer(ss.vertices[0]) || graph.getModel().getChildCount(ss.vertices[0]) > 0));
  ...
  this.actions.get("collapsible").setEnabled(ss.vertices.length == 1 && (graph.model.getChildCount(ss.vertices[0]) > 0 || graph.isContainer(ss.vertices[0])));
  ...
  // Updates menu states
  this.menus.get("navigation").setEnabled(ss.cells.length > 0 || graph.view.currentRoot != null);
...
EditorUi.prototype.refresh = function (sizeDidChange) {
  ...
  this.formatContainer.style.width = this.format != null && this.formatWidth > 0 ? "" : "0";

  if (sizeDidChange || sw != this.sidebarContainer.style.width || fw != this.formatContainer.style.width) {
    this.editor.graph.sizeDidChange();
  }
};
...
EditorUi.prototype.addSplitHandler = function (elt, horizontal, dx, onChange) {
  ...
  var getValue = mxUtils.bind(this, function () {
    var result = parseInt(horizontal ? elt.offsetLeft : elt.offsetTop);
...
EditorUi.prototype.pickColor = function (color, apply, defaultColor, defaultColorValue, singleColorMode) {
  ...
  var h = (singleColorMode ? 240 : 290) + (Math.ceil(ColorDialog.prototype.presetColors.length / 12) + Math.ceil(ColorDialog.prototype.defaultColors.length / 12)) * 17;
...
EditorUi.prototype.executeLayouts = function (layouts, post) {
  this.executeLayout(
    mxUtils.bind(this, function () {
      ...
      layout.execute(this.editor.graph.getDefaultParent(), cells.length == 0 ? null : cells);
    }),
    true,
    post,
  );
...
EditorUi.prototype.showDataDialog = function (cell) {
  if (cell != null && typeof window.EditDataDialog !== "undefined") {
    ...
    this.showDialog(dlg.container, 480, 420, true, false, null, false, null, new mxRectangle(0, 0, 440, 220));
    dlg.init();
  }
};
  ...
  keyHandler.isEventIgnored = function (evt) {
    ...
    return (
      ...
      (!this.isControlDown(evt) ||
        ...
        (evt.keyCode != 90 && evt.keyCode != 89 && evt.keyCode != 188 && evt.keyCode != 190 && evt.keyCode != 85 && evt.keyCode != 83)) &&
      ((evt.keyCode != 66 && evt.keyCode != 73) || !this.isControlDown(evt) || (this.graph.cellEditor.isContentEditing() && !mxClient.IS_FF && !mxClient.IS_SF)) &&
...
EditorUi.prototype.addMenuHandler = function (elt, funct, clickFn, handleKeyUp) {
  if (funct != null) {
    ...
    var clickHandler = mxUtils.bind(this, function (evt) {
      ...
      if (!mxEvent.isConsumed(evt) && show && (elt.enabled == null || elt.enabled)) {
        this.editor.graph.popupMenuHandler.hideMenu();
    ...
    mxEvent.addListener(
      ...
      "mousemove",
      mxUtils.bind(this, function (evt) {
        if (this.menus.autoPopup && this.currentMenu != null && this.currentMenuElt != elt && this.currentMenuElt != null) {
    ...
    mxEvent.addListener(
      ...
      mxUtils.bind(this, function (evt) {
        if (!this.menus.autoPopup && this.currentMenu != null && this.currentMenuElt != elt && mxEvent.isMouseEvent(evt)) {
          this.hideCurrentMenu();
        }
    ...
    if (handleKeyUp) {
      ...
      mxEvent.addListener(
        ...
        mxUtils.bind(this, function (evt) {
          ...
          if (evt.keyCode != 13 /* Enter */ && evt.keyCode != 27 /* Escape */) {
            clickHandler(evt);
            show = true;
          }
        }),
      ...
      if (this.isShapePickerVisible()) {
        ...
        if (Editor.inlineFullscreen || this.embedViewport == null) {
          if (vertical) {
            ...
            off.x += 16 + elt.offsetHeight + this.sketchPickerMenuElt.offsetWidth / 2;
            off.y += 20;
          }
        } else {
          off.x = 0;
        ...
        this.showShapePicker(
          Math.max(this.diagramContainer.scrollLeft + Math.max(24, off.x)),
          this.diagramContainer.scrollTop + off.y,
          null,
          null,
          null,
          null,
...
EditorUi.prototype.createZoomInput = function (readOnly) {
  ...
  this.dependsOnLanguage(
    mxUtils.bind(this, function () {
      zoomInput.setAttribute("title", mxResources.get("zoom") + " (Alt+Mousewheel)");
    }),
  );
  ...
  mxEvent.addGestureListeners(
    ...
    mxUtils.bind(this, function (evt) {
      ...
      if (!mxClient.IS_TOUCH && mxEvent.getSource(evt) == zoomInput && document.activeElement != zoomInput) {
        zoomInput.focus();
    ...
    mxUtils.bind(this, function (evt) {
      if (mxEvent.getSource(evt) == zoomInput && consumeEvent) {
        mxEvent.consume(evt);
      }
    }),
    mxUtils.bind(this, function (evt) {
