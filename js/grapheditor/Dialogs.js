...
var ColorPicker = function () {
  ...
  function fromHSV(h, s, v) {
    ...
    return hsv2rgb(h == null ? currentHsv[0] : (currentHsv[0] = h), s == null ? currentHsv[1] : (currentHsv[1] = s), v == null ? currentHsv[2] : (currentHsv[2] = v));
  }
  ...
  var update = mxUtils.bind(this, function (quiet) {
    ...
    if (arr != null) {
      for (var i = 0; i < seg.length; i++) {
        seg[i].style.backgroundColor = "rgb(" + arr[0] * (1 - i / seg.length) * 255 + ", " + arr[1] * (1 - i / seg.length) * 255 + ", " + arr[2] * (1 - i / seg.length) * 255 + ")";
      }
      ...
      var color =
        ...
        (currentAlpha < 1 ? ("0" + Math.round(currentAlpha * 255).toString(16)).slice(-2) : "");

      if (quiet != true) {
        this.fireEvent(new mxEventObject("change", "color", mxUtils.rgba2hex(color)));
      }
    }
  });
  ...
  this.fromString = function (color, quiet) {
    ...
    if (color == null || color == "" || mxUtils.isVarColor(color) || color == mxConstants.NONE) {
      color = "#ffffff";
      quiet = true;
    }
...
var ColorDialog = function (editorUi, color, apply, cancelFn, defaultColor, defaultColorValue, singleColorMode) {
  var cssDefaultColor = defaultColorValue != null ? mxUtils.getLightDarkColor(defaultColorValue, null, null, singleColorMode) : null;
  ...
  darkSelect.value = !mxUtils.isLightDarkColor(color) ? "automatic" : "custom";
  ...
  mxEvent.addListener(opacityBtn, "click", function (evt) {
    ...
    editorUi.prompt(mxResources.get("opacity") + " (0-100)", opacity, function (newValue) {
      if (!isNaN(newValue) && newValue !== "" && newValue >= 0 && newValue <= 100) {
        ...
        input.value = mxUtils
          .rgba2hex(mxUtils.addAlphaToColor("#" + input.value, newValue / 100, true))
          .substring(1)
          .toUpperCase();
        darkInput.value = mxUtils
          .rgba2hex(mxUtils.addAlphaToColor("#" + darkInput.value, newValue / 100, true))
          .substring(1)
          .toUpperCase();
  ...
  function isDefaultColor(color) {
    ...
    return (
      defaultColor != null &&
      (color == "" || lc == "automatic" || lc == "default" || lc == mxResources.get("default").toLowerCase() || lc == mxResources.get("automatic").toLowerCase())
    );
  }

  function updateInputColor(input) {
    if (input.value == "" && cssDefaultColor != null && useDefault) {
      input.style.background = input == darkInput ? cssDefaultColor.dark : cssDefaultColor.light;
    } else if (input.value == "" || input.value == mxConstants.NONE || mxUtils.isVarColor(input.value)) {
    ...
    if (input.style.background == "transparent") {
      ...
      input.style.color = mxUtils.isDarkColor(input.style.background) ? "#ffffff" : "#000000";
    }
  }
    ...
    if (useDefault != selected) {
      if (selected) {
        ...
        if (darkInput.value == "" && cssDefaultColor != null) {
          ...
          darkInput.value = mxUtils.rgba2hex(style.backgroundColor).substring(1).toUpperCase();
        }
  ...
  var defaultBtn = mxUtils.button("", function () {
    ...
    if (currentInput.value == "" && cssDefaultColor != null && useDefault) {
      picker.fromString(currentInput == darkInput ? cssDefaultColor.dark : cssDefaultColor.light, true);
    }
  });
  ...
  function inputChanged(elt) {
    ...
    if (isDefaultColor(elt.value) && cssDefaultColor != null) {
      ...
      if (elt.value != mxConstants.NONE && !mxUtils.isVarColor(elt.value)) {
      ...
      if (elt == darkInput && (input.value != mxConstants.NONE || darkInput.value != mxConstants.NONE)) {
        darkSelect.value = "custom";
      }
      ...
      if (elt == input && darkSelect.value == "automatic") {
        if (input.value == mxConstants.NONE) {
          ...
          darkInput.value = mxUtils
            .rgba2hex(mxUtils.getInverseColor("#" + elt.value))
            .substring(1)
            .toUpperCase();
        }
      }
  ...
  function doApply() {
    ...
    if (
      (useDefault && cssDefaultColor != null) ||
      ((/(^#?[a-zA-Z0-9]*$)/.test(color) || mxUtils.isVarColor(color)) && (/(^#?[a-zA-Z0-9]*$)/.test(dark) || mxUtils.isVarColor(dark)))
    ) {
      if (useDefault) {
        color = "default";
      } else {
        ...
        if (color != mxConstants.NONE && !mxUtils.isVarColor(color) && color.charAt(0) != "#") {
          color = "#" + validateColor(color);
        }

        if (dark != mxConstants.NONE && !mxUtils.isVarColor(dark) && dark.charAt(0) != "#") {
          dark = "#" + validateColor(dark);
        }

        var recentCount = 12;

        if (color == mxConstants.NONE || dark == mxConstants.NONE) {
          ColorDialog.addRecentColor(mxConstants.NONE, recentCount);
        }

        if (darkSelect.value != "automatic" && dark != mxConstants.NONE) {
  ...
  var currentInput =
    Editor.isDarkMode() && (graph.getAdaptiveColors() == "auto" || (graph.getAdaptiveColors() == "simple" && darkSelect.value == "custom")) && !singleColorMode ? darkInput : input;

  function updatePickerFromCurrentInput() {
    if (currentInput.value != mxConstants.NONE && currentInput.value != "") {
          ...
          if (clr != null) {
            ...
            if (clr == mxConstants.NONE || clr == null) {
              ...
              if (mxUtils.isLightDarkColor(clr)) {
                ...
                td.style.background = "linear-gradient(to right bottom, " + cssColor.cssText + " 50%, " + mxUtils.invertLightDarkColor(cssColor).cssText + " 50.3%)";
              } else {
          ...
          if (clr != null) {
            ...
            mxEvent.addListener(td, "click", function () {
              if (clr == mxConstants.NONE) {
                if (currentInput == darkInput && darkInput.value == mxConstants.NONE) {
                  input.value = mxConstants.NONE;
                } else {
                  currentInput.value = mxConstants.NONE;
                }

                if (currentInput == input || (currentInput == darkInput && input.value == mxConstants.NONE)) {
                  darkSelect.value = "automatic";
                }
  ...
  if (!singleColorMode) {
    ...
    if (ColorDialog.collapsed && (!Editor.isDarkMode() || graph.getAdaptiveColors() != "auto") && darkSelect.value == "automatic") {
      darkColorDiv.style.display = "none";
      updateCollapseIcon();
    }
  ...
  if (cssDefaultColor != null) {
    defaultBtn.setAttribute("title", mxResources.get(singleColorMode ? "default" : "useBlackAndWhite"));
    ...
    if (singleColorMode) {
      ...
      def.style.backgroundImage = "linear-gradient(to right bottom, " + cssDefaultColor.cssText + " 50%, " + mxUtils.invertLightDarkColor(cssDefaultColor).cssText + " 50.3%)";
    }
  ...
  if (color != null) {
    if (color == "default") {
      ...
      if (color == mxConstants.NONE || cssColor.light == "transparent") {
        input.value = mxConstants.NONE;
      } else {
      ...
      if (color == mxConstants.NONE || cssColor.dark == "transparent") {
        darkInput.value = mxConstants.NONE;
      } else {
  ...
  if (!singleColorMode && ColorDialog.collapsed && darkSelect.value == "automatic" && !Editor.isDarkMode()) {
    darkColorDiv.style.display = "none";
    updateCollapseIcon();
  }
...
var SimpleTextareaDialog = function (editorUi, value, fn, buttonLabel, helpLink) {
  ...
  var okBtn = mxUtils.button(buttonLabel != null ? buttonLabel : mxResources.get("apply"), function () {
    editorUi.hideDialog();
    fn(textarea.value);
  });
 ...
 */
var TextareaDialog = function (editorUi, title, url, fn, cancelFn, cancelTitle, w, h, addButtons, noHide, noWrap, applyTitle, helpLink, customButtons, header) {
  w = w != null ? w : 300;
  h = h != null ? h : 120;
...
var EditDiagramDialog = function (editorUi) {
  ...
  if (editorUi.editor.graph.isEnabled()) {
    ...
    mxUtils.write(applyOption, mxResources.get("apply", null, "Update Existing Drawing"));
  ...
  var okBtn = mxUtils.button(mxResources.get("ok"), function () {
    ...
    if (select.value == "new") {
      ...
      try {
        ...
        if (cause) {
          EditorUi.debug("EditDiagramDialogParserError", [this], "node", [node], "cause", [cause]);
  ...
  if (format == "xml") {
    ...
    if (param.length <= MAX_REQUEST_SIZE && w * h < MAX_AREA) {
      ...
      var req = new mxXmlRequest(
        EXPORT_URL,
        "format=" + format + "&filename=" + encodeURIComponent(name) + "&bg=" + (bg != null ? bg : "none") + "&w=" + w + "&h=" + h + "&" + param + "&dpi=" + dpi,
      );
...
var EditDataDialog = function (ui, cell) {
  ...
  for (var i = 0; i < attrs.length; i++) {
    if ((attrs[i].nodeName != "label" || style["metaEdit"] == "1" || Graph.translateDiagram || isLayer) && attrs[i].nodeName != "placeholders") {
  ...
  if (ui.editor.graph.getModel().isVertex(cell) || ui.editor.graph.getModel().isEdge(cell)) {
    ...
    if (EditDataDialog.placeholderHelpLink != null) {
      replace.appendChild(ui.createHelpIcon(EditDataDialog.placeholderHelpLink));
    }
...
var LayersWindow = function (editorUi, x, y, w, h) {
      ...
      var menu = new mxPopupMenu(
        mxUtils.bind(this, function (menu, parent) {
          ...
          menu
            ...
            .setAttribute("title", selectedLayers.length > 0 ? mxResources.get("selectNone") : nothingIsSelected);

          menu.addItem(
            mxResources.get("invertSelection"),
            null,
            mxUtils.bind(this, function () {
              refresh();
              selectLayers();
          ...
          menu
            ...
            .setAttribute("title", selectedLayers.length > 0 ? mxResources.get("show") : nothingIsSelected);
          menu
            .addItem(
              mxResources.get("hide"),
              null,
              mxUtils.bind(this, function () {
            ...
            .setAttribute("title", selectedLayers.length > 0 ? mxResources.get("hide") : nothingIsSelected);

          menu.addSeparator(layersSubmenu);

          menu
            .addItem(
              mxResources.get("lock"),
              null,
              mxUtils.bind(this, function () {
            ...
            .setAttribute("title", selectedLayers.length > 0 ? mxResources.get("lock") : nothingIsSelected);

          menu
            .addItem(
              mxResources.get("unlock"),
              null,
              mxUtils.bind(this, function () {
            ...
            .setAttribute("title", selectedLayers.length > 0 ? mxResources.get("unlock") : nothingIsSelected);
          ...
          menu.addItem(
            mxResources.get("selectObjectsInLayer"),
            null,
            mxUtils.bind(this, function () {
            ...
            layerSubmenu,
            null,
            selectionLayer.children != null && selectionLayer.children.length > 0,
          );

          // Adds submenu for moving selection cells
          var moveSubmenu = menu.addItem(mxResources.get("moveSelectionTo", [""]), null, null, parent, null, !graph.isSelectionEmpty());
  ...
  function refresh() {
    ...
    function addLayer(index, label, child, defaultParent, selected) {
      ...
      ldiv.setAttribute("title", label + " (" + child.getId() + ")");
      ...
      mxEvent.addListener(ldiv, "dragend", function (evt) {
        if (dragSource != null && dropIndex != null && model.getChildCount(model.root) > 1) {
      ...
      inp.setAttribute("title", mxResources.get(visible ? "hide" : "show"));
        ...
        try {
          ...
          if (allLayers) {
            ...
            for (var i = 0; i < model.getChildCount(parent); i++) {
              graph.setCellStyles("locked", value, [model.getChildAt(parent, i)]);
            }
          } else {
      ...
      mxEvent.addListener(btn, "click", function (evt) {
        if (graph.isEnabled()) {
          setLayersLocked(mxEvent.isShiftDown(evt) ? getSelectedLayers(null, true) : [child], !locked);
          mxEvent.consume(evt);
        }
      });
      ...
      mxEvent.addListener(ldiv, "dblclick", function (evt) {
        ...
        if (nodeName != "INPUT" && nodeName != "IMG" && title.contentEditable != "true") {
          renameLayer(child);
          mxEvent.consume(evt);
        }
      });
      ...
      mxEvent.addListener(ldiv, "click", function (evt) {
        if (graph.isEnabled() && title.contentEditable != "true" && mxEvent.getSource(evt) != cb) {
