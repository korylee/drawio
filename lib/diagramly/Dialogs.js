...
import {
  ...
  mxResources,
  mxUtils,
} from "../mxgraph/src/index.js";
import { App } from "./App.js";
import { EditorUi } from "../grapheditor/EditorUi.js";
import { Graph } from "../grapheditor/Graph.js";
import { Editor, FilenameDialog } from "../grapheditor/Editor.js";
import { ColorDialog } from "../grapheditor/Dialogs.js";
import Spinner from "../spinner/index.js";

export function StorageDialog(editorUi, fn, rowLimit) {
 ...
 */
export function SplashDialog(editorUi) {
  var div = document.createElement("div");
  div.style.textAlign = "center";
...
export function EmbedDialog(
  ...
  if (
    ...
    previewBtn = mxUtils.button(
      previewTitle != null
        ? previewTitle
        : mxResources.get(
            result.length < maxSize ? "preview" : "openInNewWindow",
          ),
      function () {
            ...
            if (doc != null) {
              doc.writeln(
                "<html><head><title>" +
                  ...
                  (result.substring(0, 7) == "<iframe"
                    ? result
                    : mxUtils.htmlEntities(result)) +
                  "</body></html>",
              );
              doc.close();
            } else {
  ...
  var downloadBtn = mxUtils.button(mxResources.get("export"), function () {
    editorUi.hideDialog();
    editorUi.saveData(
      filename != null ? filename : "embed.txt",
      "txt",
      result,
      "text/plain",
    );
  ...
  if (!editorUi.isOffline() && result.length < maxSize) {
    var emailBtn = mxUtils.button("", function () {
      try {
        var url =
          "mailto:?subject=" +
          encodeURIComponent(filename || editorUi.defaultFilename) +
          "&body=" +
 ...
 */
export function CreateGraphDialog(editorUi, title, type) {
  var div = document.createElement("div");
...
export function BackgroundImageDialog(
  ...
  applyBtn = mxUtils.button(mxResources.get("apply"), function () {
    ...
    urlChanged(null, function (url) {
      applyFn(
        url != "" && url != null
          ? new mxImage(url, widthInput.value, heightInput.value)
          : null,
        url == null,
        newBackgroundColor,
 ...
 * Constructs a new parse dialog.
 */
export function ParseDialog(editorUi, title, defaultType) {
  var plantUmlExample =
  ...
  function getDefaultValue() {
    if (typeSelect.value == "list") {
      ...
      typeSelect.value == "mermaid2drawio"
    ) {
      return "graph TD;\n  A-->B;\n  A-->C;\n  B-->D;\n  C-->D;";
  ...
  if (
    defaultType == "plantUml" &&
    window.PLANT_URL == "https://plant-aws.diagrams.net"
  ) {
  ...
  if (
    (!editorUi.isOffline() || mxClient.IS_CHROMEAPP) &&
    (defaultType == "mermaid" || defaultType == "plantUml")
  ) {
    buttons.appendChild(
      editorUi.createHelpIcon(
        defaultType == "mermaid"
...
export function NewDialog(
  ...
  leftHighlight =
    leftHighlight != null
      ? leftHighlight
      : "light-dark(#ebf2f9, " + Editor.darkColor + ")";
  rightHighlight =
    rightHighlight != null ? rightHighlight : "light-dark(#e6eff8, #ffffff)";
  rightHighlightBorder =
  ...
  function create() {
    if (
      ...
      if (
        ...
        editorUi.spinner.spin(
          document.body,
          mxResources.get("generate") + " '" + generateInput.value + "'",
        )
      ) {
  ...
  tmplSearchInput.style.cssText =
    "width:105px;height:16px;border:1px solid #d3d3d3;" +
    ...
    if (selectedElt != elt) {
      if (selectedElt != null) {
        ...
        if (selectedElt == generateElt) {
          ...
          magnifyGenerate.style.visibility =
            lastAiXml != null ? "visible" : "hidden";
        }
      }
  ...
  generateInput.setAttribute(
    "placeholder",
    mxResources.get("describeYourDiagram"),
  );
  ...
  var mouseUpHandler = function (evt) {
    if (!generatePreviewWasVisible && lastAiXml != null) {
      var previewXml =
        "<mxfile><diagram>" + Graph.compress(lastAiXml) + "</diagram></mxfile>";
      showTooltip(
        previewXml,
    ...
    function loadXmlData(url, callback) {
      if (xmlData == null) {
        ...
        if (/^https?:\/\//.test(realUrl)) {
          realUrl = editorUi.editor.isCorsEnabledForUrl(realUrl)
            ? realUrl
  ...
  function filterTemplates() {
    ...
    var temp = mxResources.get(
      results.length == 0 ? "noResultsFor" : "resultsFor",
      [searchTerms],
    );
  ...
  function initUi() {
    ...
    function getEntryTitle(cat, templateList) {
      ...
      return (
        label + (templateList != null ? " (" + templateList.length + ")" : "")
      );
    }
            ...
            if (typeof node.getAttribute !== "undefined") {
              if (node.nodeName == "parsererror") {
                if (window.console != null) {
                  console.log(
                    "Parser error in " + templateFile + ": " + node.textContent,
                  );
                }
                ...
                if (url != null) {
                  ...
                  var tempObj = {
                    ...
                    tooltip:
                      node.getAttribute("name") || node.getAttribute("url"),
...
export function SaveDialog(
  ...
  if (
    ...
    if (
      Editor.popupsAllowed &&
      (disabledModes == null || mxUtils.indexOf(disabledModes, "_blank") < 0)
    ) {
  ...
  if (
    ...
    typeSelect = FilenameDialog.createFileTypes(
      editorUi,
      saveAsInput,
      editorUi.editor.diagramFileTypes,
    );
    ...
    if (
      ...
      if (isLocal || editorUi.getServiceForName(mode) != null) {
        ...
        if (
          !isLocal &&
          editorUi.isOffline(
            mode == App.MODE_GOOGLE && urlParams["gapi-stealth"] == "1",
          )
        ) {
          option.setAttribute("disabled", "disabled");
        ...
        if (entryType == "pick") {
          ...
          option.setAttribute(
            "title",
            title + " - " + mxResources.get("pickFolder") + "...",
          );
        } else {
          ...
          option.innerHTML =
            mxUtils.htmlEntities(title) +
            (shortPath != null ? dash + mxUtils.htmlEntities(shortPath) : "") +
            state;
          option.setAttribute(
            "title",
            title +
              (path != null ? " (" + path + ")" : "") +
              (id != null && decodeURIComponent(id) != path
                ? " [" + id + "]"
                : ""),
          );
          ...
          if (
            ...
            if (entryType == "root") {
              selected =
                (defaultMode == null && editorUi.mode == mode) ||
    ...
    editorUi.pickFolder(
      ...
      function (result) {
        ...
        if (
          ...
          entry = {
            mode: mode,
            path: result.value[0].name,
            id: OneDriveFile.prototype.getIdOf(result.value[0]),
          };
        } else if (
          (mode == App.MODE_GITHUB || mode == App.MODE_GITLAB) &&
          result != null &&
          result.length > 0
        ) {
  ...
  function checkExtension() {
    if (
      ...
      var ext = editorUi.getExtensionForService(
        entries[storageSelect.value].mode,
      );
  ...
  function addStorageEntries() {
    ...
    if (recent != null && recent.length > 0) {
      for (var i = 0; i < recent.length; i++) {
        if (
          addStorageEntry(recent[i].mode, recent[i].path, recent[i].id) != null
        ) {
          recentCount++;
        }
      }
 ...
 */
export function PopupDialog(editorUi, url, pre, fallback, hideDialog) {
  hideDialog = hideDialog != null ? hideDialog : true;
...
export function ImageDialog(
  ...
  var embedBtn = mxUtils.button(mxResources.get("embed"), function () {
    if (
      linkInput.value.substring(0, 5) != "data:" &&
        ...
        editorUi.editor.convertImageToDataUri(
          ...
          function (uri) {
            editorUi.confirmImageResize(
              function (doResize) {
                editorUi.resizeImage(
                  ...
                  mxUtils.bind(this, function (data2, w2, h2) {
                    ...
                    if (
                      data2 != null &&
                      data2.length < editorUi.maxImageBytes
                    ) {
              ...
              img.width > editorUi.maxImageSize ||
                img.height > editorUi.maxImageSize ||
...
export function LinkDialog(
  ...
  if (!editorUi.isOffline()) {
    btns.appendChild(
      editorUi.createHelpIcon("https://www.drawio.com/doc/faq/custom-links"),
    );
  }
 ...
 */
export function RevisionDialog(editorUi, revs, restoreFn) {
  var div = document.createElement("div");
  ...
  var compareBtn = editorUi.createToolbarButton(
    Editor.compareImage,
    mxResources.get("compare"),
  );
  compareBtn.setAttribute("disabled", "disabled");

  var mergeBtn = editorUi.createToolbarButton(
  ...
  mxEvent.addGestureListeners(
    ...
    function (e) {
      ...
      try {
        var curr =
          diagrams[currentPage] != null
            ? currentDiagrams[diagrams[currentPage].getAttribute("id")]
            : null;
        ...
        mxUtils.write(
          errorNode,
          mxResources.get("pageNotFound") + ": " + e.message,
        );
      }
    },
    null,
    ...
    mxUtils.bind(this, function (e) {
      if (currentDoc != null) {
        ...
        var dlg = new TextareaDialog(
            ...
            if (patch[EditorUi.DIFF_UPDATE] != null) {
              ...
              mxEvent.addListener(patchSelect, "change", function (evt) {
                ...
                input.value = JSON.stringify(
                  pagePatch == null ? patch : pagePatch,
                  null,
                  2,
                );
              });
          ...
          null,
          mxResources.get(
            editorUi.editor.graph.isEnabled() ? "merge" : "close",
          ),
        );
  ...
  var createBtn = mxUtils.button(
    mxResources.get("createRevision"),
    function () {
          ...
          function updateGraph(xml) {
            ...
            if (node != null) {
              ...
              function parseGraphModel(dataNode) {
                ...
                graph.setAdaptiveColors(
                  dataNode.getAttribute("adaptiveColors"),
                );
              ...
              if (node.nodeName == "mxfile") {
                ...
                if (diagrams.length > 1) {
                  ...
                  for (var i = 0; i < diagrams.length; i++) {
                    ...
                    pageOption.setAttribute(
                      "title",
                      name + " (" + diagrams[i].getAttribute("id") + ")",
                    );
                    pageOption.setAttribute("value", i);
                    var name =
                      diagrams[i].getAttribute("name") ||
                      mxResources.get("pageWithNumber", [i + 1]);
                    var localPage = editorUi.getPageById(
                    ...
                    if (localPage != null) {
                      ...
                      if (
                        editorUi.getHashValueForPages([localPage]) !=
                        editorUi.getHashValueForPages([newPage])
                      ) {
                        state = " (M)";
                      }
                    } else {
 ...
 *
 */
export function FindWindow(ui, x, y, w, h, withReplace) {
  ...
  var help = ui.menus.createHelpLink(
    "https://www.drawio.com/doc/faq/find-shapes",
  );
    ...
    if (typeof cell.value === "object" && cell.value.attributes != null) {
      ...
      for (var i = 0; i < attrs.length; i++) {
        if (attrs[i].nodeName != "label") {
          ...
          if (
            (re == null && value.indexOf(search) >= 0) ||
          ...
          if (
            ...
            if (withReplace) {
              if (re != null) {
                ...
                lblMatch = searchStr;
                lblMatchPos =
                  lblPosShift + label.indexOf(searchStr) + lblMatch.length;
              }
            }
  ...
  if (!withReplace) {
    ...
    function replaceInLabel(str, substr, newSubstr, startIndex, style) {
      ...
      str = str
        .replace(/<br>/gi, "\n")
        .replace(
          /(\s|\S)(<(BLOCKQUOTE|DIV|H1|H2|H3|H4|H5|H6|OL|P|PRE|TABLE|UL)[^>]*>)/gi,
          "$1\n$2",
        );
      ...
      return newStr
        .replace(
          /\n(<(BLOCKQUOTE|DIV|H1|H2|H3|H4|H5|H6|OL|P|PRE|TABLE|UL)[^>]*>)/gi,
          "$1",
        )
        .replace(/\n/g, "<br>");
    }
 ...
 *
 */
export function FreehandWindow(editorUi, x, y, w, h, withBrush) {
  var graph = editorUi.editor.graph;
  ...
  if (withBrush) {
    ...
    function updateName() {
      ...
      if (
        ...
        if (color == "default") {
          ...
          var clr =
            color.charAt(0) == "#" ? color.substring(1).toUpperCase() : color;
    ...
    mxEvent.addListener(
      ...
      mxUtils.bind(this, function (evt) {
        ...
        editorUi.prompt(
          mxResources.get("smoothing") + " (1-20)",
          smoothing,
          function (newValue) {
  ...
  graph.addListener(
    ...
    mxUtils.bind(this, function () {
      ...
      mxUtils.write(
        startBtn,
        mxResources.get(
          graph.freehand.isDrawing() ? "stopDrawing" : "startDrawing",
        ),
      );
 ...
 *
 */
export function AdaptiveColorsWindow(editorUi, x, y, w, h) {
  var graph = editorUi.editor.graph;
  ...
  var btn = mxUtils.button(
    ...
    mxUtils.bind(this, function () {
      editorUi.removeUserDefinedDarkColors(
 ...
 *
 */
export function ChatWindow(editorUi, x, y, w, h) {
  var graph = editorUi.editor.graph;
          ...
          if (data[1].length > 0) {
            ...
            if (editorUi.getServiceName() == "draw.io") {
              ...
              btn.setAttribute(
                "title",
                mxResources.get(
                  !editorUi.isStandaloneApp() ? "openInNewWindow" : "export",
                ),
              );
              buttons.appendChild(btn);
 ...
 *
 */
export function TagsWindow(editorUi, x, y, w, h) {
  var graph = editorUi.editor.graph;
  var helpButton = null;

  if (!editorUi.isOffline() || mxClient.IS_CHROMEAPP) {
    helpButton = editorUi.menus.createHelpLink(
 ...
 */
export function AuthDialog(editorUi, peer, showRememberOption, fn) {
  var div = document.createElement("div");
  div.style.textAlign = "center";
...
export function MoreShapesDialog(editorUi, expanded, entries) {
  ...
  if (editorUi.sidebar.customEntries != null) {
    for (var i = 0; i < editorUi.sidebar.customEntries.length; i++) {
      ...
      for (
        var j = 0;
        section.entries != null && j < section.entries.length;
        j++
      ) {
  ...
  if (expanded) {
    ...
    var applyBtn = mxUtils.button(mxResources.get("apply"), function () {
      ...
      if (
        ...
        if (
          (editorUi.scratchpad != null) !=
          (idx >= 0 && libs.splice(idx, 1).length > 0)
        ) {
...
export function PluginsDialog(editorUi, addFn, delFn, closeOnly) {
  var div = document.createElement("div");
  var inner = document.createElement("div");
  ...
  var applyBtn = mxUtils.button(
    closeOnly ? mxResources.get("close") : mxResources.get("apply"),
    function () {
      if (changed) {
  ...
  if (!editorUi.isOffline()) {
    buttons.appendChild(
      editorUi.createHelpIcon("https://www.drawio.com/doc/faq/plugins"),
    );
  }
...
export function CropImageDialog(editorUi, image, clipPath, fn) {
  var IMAGE_SIZE = 300;
  var div = document.createElement("div");
  ...
  var cropGraph = null,
    ...
    commonStyle =
      "shape=image;fillColor=none;rotatable=0;cloneable=0;deletable=0;image=" +
...
export function EditGeometryDialog(editorUi, vertices) {
  var graph = editorUi.editor.graph;
      ...
      if (mimeType == null || mimeType.substring(0, 6) == "image/") {
        if ((data == null && img != null) || entries[data] == null) {
          ...
          if (data != null) {
            ...
            var cells = editorUi.stringToCells(
        ...
        try {
          ...
          if (doc.documentElement.nodeName == "mxlibrary") {
            ...
            for (var i = 0; i < pages.length; i++) {
              ...
              if (cells.length > 0) {
                var size =
                  editorUi.editor.graph.getBoundingBoxFromGeometry(cells);
  ...
  var btn = mxUtils.button(mxResources.get("export"), function () {
    ...
    if (editorUi.isLocalFileSave()) {
      editorUi.saveLocalFile(
        ...
        null,
        null,
        allowBrowser != null ? allowBrowser : true,
        null,
        "xml",
      );
    } else {
 ...
 */
export function EditShapeDialog(editorUi, cell, title) {
  var div = document.createElement("div");
  div.style.position = "absolute";
  div.style.left = "30px";
 ...
 */
export function BtnDialog(editorUi, peer, btnLbl, fn) {
  var div = document.createElement("div");
  div.style.textAlign = "center";
 ...
 */
export function FontDialog(editorUi, curFontname, curUrl, curType, fn) {
  var row, td, label;
  ...
  if (!editorUi.isOffline()) {
    td.appendChild(
      editorUi.createHelpIcon("https://www.drawio.com/blog/external-fonts"),
    );
  }
 ...
 */
export function AspectDialog(editorUi, pageId, layerIds, okFn, cancelFn) {
  this.aspect = {
...
AspectDialog.prototype.createViewer = function (
  container,
  pageNode,
  layerId,
  defaultBackground,
...
AspectDialog.prototype.createLayerItem = function (
  ...
  var onClick = mxUtils.bind(this, function () {
    if (
      $listItem.className.indexOf("geAspectDlgListItemSelected") >= 0
    ) //Selected
    {
      $listItem.className = "geAspectDlgListItem";
 ...
 */
export function FilePropertiesDialog(editorUi, publicLink) {
  var row, td;
  var table = document.createElement("table");
  var tbody = document.createElement("tbody");
  table.style.width = "100%";
  ...
  if (isPng || isSvg) {
    ...
    var initialCompressed =
      file != null ? file.isCompressed() : Editor.defaultCompressed;
    ...
    addApply(function (success, error) {
      if (
        ...
        initialCompressed != compressedInput.checked
      ) {
        window.setTimeout(function () {
          editorUi.fileNode.setAttribute(
            "compressed",
  ...
  var genericBtn = mxUtils.button(mxResources.get("apply"), function () {
    if (
      editorUi.spinner.spin(document.body, mxResources.get("updatingDocument"))
    ) {
      apply(
        function () {
          editorUi.spinner.stop();
...
export function ConnectionPointsDialog(editorUi, cell) {
  var GRAPH_SIZE = 350,
    CP_SIZE = 6,
    CP_HLF_SIZE = 3;
  var div = document.createElement("div");
  ...
  this.init = function () {
    ...
    function createCPoint(x, y, constObj) {
      var cPointStyle =
        "shape=image;points=[];rotatable=0;resizable=0;connectable=0;editable=0;image=" +
        editorUi.convertDataUri(mxConstraintHandler.prototype.pointImage.src) +
        ";";
      var cPoint = new mxCell(
        "",
        new mxGeometry(x, y, CP_SIZE, CP_SIZE),
        cPointStyle,
      );
      cPoint.vertex = true;
      cPoint.cp = true;
    ...
    editingGraph.dblClick = function (evt, cell) {
      if (cell != null && cell != mainCell) {
        ...
        editingGraph.setSelectionCell(
          createCPoint(
            (pt.x - CP_HLF_SIZE * scale) / scale - tr.x,
    ...
    var deleteBtn = editorUi.createToolbarButton(
      Editor.trashImage,
    ...
    function applyPointProp() {
      ...
      var constObj = new mxConnectionConstraint(
        new mxPoint(x / 100, y / 100),
        false,
        null,
        dx,
        dy,
    ...
    function getConstraintFromCPoint(cp) {
      ...
      var x = mxUtils.format(
        (cp.geometry.x + CP_HLF_SIZE - mGeo.x) / mGeo.width,
      );
      var y = mxUtils.format(
        (cp.geometry.y + CP_HLF_SIZE - mGeo.y) / mGeo.height,
      );
    ...
    var applyBtn = mxUtils.button(mxResources.get("apply"), function () {
      ...
      constraints.sort(function (a, b) {
        return a.x != b.x
          ...
          : a.y != b.y
            ...
            : a.dx != b.dx
              ? a.dx - b.dx
              : a.dy - b.dy; //Sort based on x then y, dx and dy
      });
      ...
      editorUi.editor.graph.setCellStyles(
        "points",
        "[" + points.join(",") + "]",
        [cell],
      );
