...
import "./Init.js";
import "../grapheditor/Init.js";

import "../grapheditor/Editor.js";
import "../grapheditor/EditorUi.js";
...
import { Editor, OpenFile } from "../grapheditor/Editor.js";
...
import { PopupDialog, SplashDialog, StorageDialog } from "./Dialogs.js";
import { LocalFile } from "./LocalFile.js";
import { OpenDialog } from "../grapheditor/Dialogs.js";
 ...
 */
App.PUSHER_URL =
  window.PUSHER_URL || "https://js.pusher.com/7.0.3/pusher.min.js";
...
(function () {
  if (!mxClient.IS_CHROMEAPP) {
    if (urlParams["offline"] != "1") {
      ...
      if (
        window.location.hostname == "db.draw.io" &&
        urlParams["mode"] == null
      ) {
...
App.main = function (callback, createUi) {
  try {
    ...
    if (window.mxIsElectron && typeof electron === "undefined") {
    ...
    if (
      ...
      urlParams["libs"] == "aws3" &&
      document.referrer.substring(0, 42) ==
        ...
        if (
          ...
          navigator.serviceWorker.getRegistration().then(function (reg) {
            ...
            // Skips service worker install on first load
              ...
              (mxSettings.settings != null && !mxSettings.settings.isNew) ||
              urlParams["enableSW"] == "1"
            ) {
              ...
              EditorUi.debug(
                "App.main",
                "Skipping service worker (first load)",
              );
            }
          });
        }
          ...
          if (
            ...
            try {
              var value = JSON.parse(
                Graph.decompress(window.location.hash.substring(9)),
              );
...
App.prototype.init = function () {
  ...
  if (this.menubar != null) {
    if (
      ...
      (Editor.currentTheme != "simple" || urlParams["embed"] != "1")
    ) {
    ...
    if (
      (Editor.currentTheme == "kennedy" && urlParams["embed"] == "1") ||
  ...
  try {
    if (
      ...
      for (var i = 0; i < window.location.ancestorOrigins.length; i++) {
        ...
        if (
          message.endsWith("dan6v7pm1f1a1.cloudfront.net") ||
          message.endsWith(
            "confluence-cloud-excalidraw-2-ll3likebca-uc.a.run.app",
          )
        ) {
          return true;
        }
    ...
    if (remember) {
      try {
        if (isLocalStorage) {
          ...
          expiry.setYear(expiry.getFullYear() + 1);
          document.cookie =
            "MODE=" + mode + "; expires=" + expiry.toUTCString();
        }
      } catch (e) {
      ...
      if (
        ...
        window.opener.openFile.setConsumer(
          mxUtils.bind(this, function (xml, filename, temp) {
            ...
            var dot = !Editor.useCanvasForExport
          ...
          if (urlParams["demo"] == "1") {
            ...
            if (waiting) {
              ...
              if (
                ...
                urlParams["splash"] != "0" ||
                (urlParams["mode"] != null && !EditorUi.isElectronApp)
              ) {
                this.loadFile();
              } else {
                this.createFile(
        ...
        if (
          ...
          if (value.charAt(0) == "{") {
              ...
              this.createFile(
                ...
                mxUtils.bind(this, function () {
                  ...
                  window.history.replaceState(
                    null,
                    null,
                    window.location.pathname + this.getSearch(["create"]),
                  );
            ...
            if (
              ...
              var graph = this.createTemporaryGraph(
              ...
              this.handleError(
                {
                  message: mxResources.get("invalidCallFnNotFound", [
                    value.type,
                  ]),
                },
                mxResources.get("errorLoadingFile"),
          ...
          if (this.stateArg.ids != null) {
            if (window.history && window.history.replaceState) {
              ...
              window.history.replaceState(
                null,
                null,
                window.location.pathname + this.getSearch(["state", "splash"]),
              );
            }
  ...
  try {
    ...
    if (mode == App.MODE_GOOGLE) {
      ...
      if (peer != null) {
        ...
        this.hideDialog();
        window.openNew =
          this.getCurrentFile() != null && !this.isDiagramEmpty();
    ...
    var fn = mxUtils.bind(this, function () {
      ...
      if (replace == null && currentFile != null) {
        replace =
          ...
          (currentFile.getMode() == null || EditorUi.isElectronApp);
      }
    ...
    if (
      ...
      if (Editor.currentTheme != "sketch" && Editor.currentTheme != "simple") {
        if (this.commentButton == null) {
          ...
          this.commentButton.style.backgroundImage =
        ...
        this.commentButton.style.display = this.commentsSupported()
          ? ""
          : "none";
      }
...
App.prototype.fetchAndShowNotification = function (target, subtarget) {
  ...
  if (
    cachedNotif == null ||
    cachedNotif.ts + 24 * 60 * 60 * 1000 < Date.now()
  ) //Cache for one day
  {
...
App.prototype.updateHeader = function () {
  if (this.menubar != null) {
    ...
    fullscreenElement.style.backgroundImage =
    ...
    var toggleFormatPanel = mxUtils.bind(this, function () {
      if (this.formatWidth > 0) {
        toggleFormatElement.style.backgroundImage =
          "url('" + Editor.rightPanelCloseImage + "')";
      } else {
        toggleFormatElement.style.backgroundImage =
          "url('" + Editor.rightPanelOpenImage + "')";
      }
    });
    ...
    toggleElement.style.backgroundImage =
      "url('" +
      (this.compactMode ? Editor.chevronDownImage : Editor.chevronUpImage) +
      "')";
    ...
    if (urlParams["embed"] != "1" && Editor.currentTheme != "atlas") {
      ...
      if (
        !mxClient.IS_FF &&
        screen.height <= 740 &&
        typeof toggleElement.click !== "undefined"
      ) {
        window.setTimeout(
          mxUtils.bind(this, function () {
            toggleElement.click();
    ...
    this.dependsOnLanguage(
      mxUtils.bind(this, function () {
        ...
        toggleFormatElement.setAttribute(
          "title",
...
App.prototype.setCompactMode = function (active, remember, delay) {
  var node =
    mxUtils.isAncestorNode(document.body, this.container) || this.editor == null
      ? this.container
  ...
  window.setTimeout(
    mxUtils.bind(this, function () {
      mxUtils.setPrefixedStyle(
        ...
        "transition",
        "all " + delay + "s ease-in-out",
      );
      mxUtils.setPrefixedStyle(
        this.toolbarContainer.style,
        "transition",
        "all " + delay + "s ease-in-out",
      );
    ...
    if (this.drive != null) {
      ...
      if (driveUsers.length > 0) {
        ...
        var createUserRow = mxUtils.bind(this, function (user) {
          ...
          img.setAttribute(
            "src",
            user.pictureUrl != null ? user.pictureUrl : Editor.userImage,
          );
