...
import {mxClient, mxResources, mxUtils} from "../mxgraph/src/index.js";
    ...
    EditorUi.prototype.fileStats = function (file, node) {
        try {
            if (file != null && file.getSize() > 500000 && file.getHash() !== '' &&
                ...
                EditorUi.logEvent({
                    category: file.getMode().toUpperCase() +
                ...
                if (this.doImportVisio) {
                    ...
                    if (remote) {
                        if (VSS_CONVERT_URL != null && !this.isOffline()) {
                            ...
                            handleError({
                                message: this.getServiceName() != 'draw.io' ? mxResources.get('vsdNoConfig') :
                    ...
                    this.parseFileData(data, mxUtils.bind(this, function (xhr) {
                        if (xhr.readyState == 4) {
                            if (xhr.status >= 200 && xhr.status <= 299 &&
                                ...
                                this.handleError({
                                    message: xhr.status == 413 ? mxResources.get('diagramTooLarge') :
                                        mxResources.get('unknownError')
                                });
                            }
                        }
 ...
 *
 */
export function ConfirmDialog(editorUi, message, okFn, cancelFn, okLabel, cancelLabel,
