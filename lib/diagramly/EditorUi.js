...
import { Editor } from "../grapheditor/Editor.js";
import {
  mxClient,
  mxCodec,
  mxConstants,
...
import { mxSettings } from "./Settings.js";
import { Graph } from "../graphEditor/Graph.js";
import { Menus } from "../grapheditor/Menus.js";
import Spinner from "../spinner";
import { App } from "./App.js";
import { ExportDialog } from "../grapheditor/Dialogs.js";
import { DiagramPage } from "./Pages.js";
...
(function () {
  ...
  if (Editor.isDarkMode()) {
    mxGraphView.prototype.gridColor =
  ...
  EditorUi.getSvgSubtree = function (href) {
    ...
    if (data != null) {
      ...
      if (
        svg.style != null &&
        svg.style.getPropertyValue("color-scheme") == "light dark"
      ) {
  ...
  EditorUi.prototype.createSpinner = function (x, y, size) {
    ...
    spinner.spin = function (container, label, error, timeout) {
      ...
      if (!this.active) {
        ...
        if (container == document.body && urlParams["embedInline"] == "1") {
  ...
  EditorUi.prototype.getHtml2 = function (
    ...
    var js =
      ...
      EditorUi.drawHost + "/js/viewer-static.min.js";
  ...
  EditorUi.prototype.fileStats = function (file, node) {
    try {
      if (
        ...
        EditorUi.logEvent({
          category:
            file.getMode().toUpperCase() + "-FILE-STATS-" + file.getHash(),
  ...
  EditorUi.prototype.fileLoaded = function (file, noDialogs, success) {
    EditorUi.debug("EditorUi.fileLoaded", [this], "file", [file], "noDialogs", [
      noDialogs,
    ]);
    ...
    if (file != null) {
      try {
        ...
        if (!this.isOffline() && file.getMode() != null) {
          ...
          EditorUi.logEvent({
            category:
              file.getMode().toUpperCase() + "-OPEN-FILE-" + file.getHash(),
            action: "size_" + file.getSize(),
            label:
              "autosave_" +
  ...
  EditorUi.prototype.loadLibrary = function (file, expand) {
    ...
    if (doc.documentElement.nodeName == "mxlibrary") {
      ...
      this.libraryLoaded(
        ...
        expand,
        doc.documentElement.getAttribute("tags"),
      );
    } else {
  ...
  EditorUi.prototype.libraryLoaded = function (
    ...
    if (tip != null && tip.length > 0 && file.title != ".scratchpad") {
      title.setAttribute(
        "title",
        this.getLibraryStorageHint(file) + "\n" + tip,
      );
    }
    ...
    if (file.isEditable()) {
      ...
      var addSelection = mxUtils.bind(this, function (evt) {
        if (!graph.isSelectionEmpty()) {
          ...
          evt != null &&
          mxEvent.getSource(evt).classList.contains("geLibraryButton")
        ) {
          this.showError(
            mxResources.get("error"),
  ...
  EditorUi.prototype.addLibraryEntries = function (imgs, content) {
    for (var i = 0; i < imgs.length; i++) {
      try {
        ...
        if (data != null) {
          data =
            data.substring(0, 5) == "data:" ? this.convertDataUri(data) : data;
          var s =
  ...
  EditorUi.prototype.showBanner = function (
    ...
    if (
      ...
      banner.className =
        "geBtn gePrimaryBtn geBanner" + (small ? " geSmallBanner" : "");
    ...
    if (win == null || win.document == null) {
      ...
      if (mimeType == "image/svg+xml" && !mxClient.IS_SVG) {
        win.document.write(
          prefix +
            "<pre>" +
            mxUtils.htmlEntities(data, false) +
            "</pre></body></html>",
        );
        win.document.close();
      } else {
  ...
  EditorUi.prototype.addChromelessToolbarItems = function (addButton) {
    ...
    if (
      this.editor.isExportToCanvas() &&
      this.isChromelessImageExportEnabled()
    ) {
    ...
    function updateLinkColor() {
      ...
      if (linkColor != null && linkColor != mxConstants.NONE) {
        ...
        div.style.backgroundImage =
          "url('" + Dialog.prototype.closeImage + "')";
      }
  ...
  EditorUi.prototype.showPublishLinkDialog = function (
    ...
    if (publicUrl != null) {
      ...
      mxUtils.write(
        publicLink,
        mxResources.get("publicDiagramUrl") +
          " (" +
    ...
    var transparent = !showBackgroundOption
      ? null
    ...
    if (
      ...
      currentPage = this.addCheckbox(
        div,
        mxResources.get("selectedPage") + ": " + name,
      );
    }
  ...
  EditorUi.prototype.showExportDialog = function (
    ...
    if (format == "png" || format == "jpeg" || format == "webp") {
      grid = this.addCheckbox(
        ...
        false,
        this.isOffline() || !Editor.canvasSupported,
        false,
        true,
      );
      height += 30;
    ...
    var include = this.addCheckbox(
      div,
      mxResources.get("includeCopyOfMyDiagram") + (pageCount > 1 ? ":" : ""),
      defaultInclude,
      null,
      null,
      format != "jpeg" && format != "webp",
      ...
      mxUtils.bind(this, function () {
        ...
        callback(
          ...
          (format == "png" || format == "svg") &&
            includeSelect.value == "currentPage",
          linkSelect.value,
          ...
          if (node != null && node.nodeName == "mxfile" && this.pages != null) {
            ...
            if (diagrams.length > 0) {
              ...
              for (var i = i0; i < diagrams.length; i++) {
                ...
                graph.model.execute(
                  new ChangePage(this, page, page, index, i > 0),
                );
                pages.push(page);
              }
   ...
   */
  EditorUi.prototype.createMermaidXml = function (
    mermaidData,
    config,
    imageData,
    w,
                    ...
                    mxUtils.bind(this, function () {
                      ...
                      if (enableParser) {
                        if (result.charAt(0) == "<") {
                          if (timeout.clear()) {
                            var parsed =
                              Editor.extractGraphModelFromText(result);
                            success(
  ...
  EditorUi.prototype.extractMermaidDeclaration = function (value) {
    ...
    for (var i = startLine; i < lines.length; i++) {
      ...
      if (temp.substring(0, 2) != "%%" && temp.substring(0, 9) != "classDef ") {
        lines2.push(lines[i]);
    ...
    EditorUi.debug(
      "EditorUi.extractMermaidDeclaration",
      "value",
      [value],
      "text",
      [text],
  ...
  EditorUi.prototype.getMermaidDiagramType = function (data) {
    ...
    while (
      k < lines.length &&
      (lines[k].trim().length == 0 || lines[k].substring(0, 2) == "%%")
    ) {
      k++;
    }
   ...
   */
  EditorUi.prototype.parseMermaidDiagram = function (
    data,
    config,
    success,
    error,
    ...
    this.generateMermaidImage(
      data,
      config,
      mxUtils.bind(this, function (imageData, w, h) {
        if (!returned) {
          returned = true;
   ...
   */
  EditorUi.prototype.mermaidSvgToDataUri = function (
    svg,
    success,
    error,
    parseError,
        ...
        this.loadImage(
          ...
          mxUtils.bind(this, function (img) {
            if (text.substring(0, 5) == "data:") {
              this.resizeImage(
                img,
                Editor.stripImageMetadata(text, Editor.removeImageMetadata),
  ...
  EditorUi.prototype.convertDataUri = function (uri) {
    ...
    if (uri.substring(0, 5) == "data:") {
      ...
      if (semi > 0) {
        uri =
          uri.substring(0, semi) + uri.substring(uri.indexOf(",", semi + 1));
      }
    }
    ...
    if (mimeType.substring(0, 5) == "image") {
      ...
      if (mimeType.substring(0, 9) == "image/png") {
        var xml = ignoreEmbeddedXml
          ? null
          : this.extractGraphModelFromPng(data);
                          ...
                          if (mxClient.IS_CHROMEAPP) {
                            ...
                            this.loadImage(
                              ...
                              mxUtils.bind(this, function (img) {
                                this.resizeImage(
                                  img,
                                  Editor.stripImageMetadata(
                                    e.target.result,
                                    Editor.removeImageMetadata,
                                  ),
                                  mxUtils.bind(this, function (data2, w2, h2) {
                                    barrier(
                                      index,
  ...
  EditorUi.prototype.init = function () {
    ...
    graph.getBackgroundImageObject = function (
      obj,
      resolveReferences,
      sourcePage,
    ) {
    ...
    if (this.menus != null) {
      ...
      this.menus.addPopupMenuEditItems = function (menu, cell, evt) {
        if (ui.editor.graph.isSelectionEmpty()) {
          ...
          this.addMenuItems(
            menu,
            ["-", "cut", "copy", "copyAsImage", "copyAsSvg", "duplicate", "-"],
            null,
            evt,
          );
    ...
    // Creates the spinner
    this.spinner =
      this.spinner != null ? this.spinner : this.createSpinner(null, null, 24);
    ...
    if (Editor.isSettingsEnabled()) {
      ...
      Editor.pageSizeUnit =
        unit == mxConstants.METERS ? mxConstants.MILLIMETERS : unit;
    ...
    if (Editor.isSettingsEnabled()) {
      ...
      if (
        mxSettings.settings.compactMode != null &&
  ...
  EditorUi.prototype.doSetCurrentTheme = function (value, force) {
    ...
    var noRestart =
      (Editor.currentTheme == "simple" && this.isDefaultTheme(value)) ||
      (this.isDefaultTheme(Editor.currentTheme) && value == "simple") ||
  ...
  EditorUi.prototype.installStatusMinimizer = function (parent) {
    ...
    var statusChanged = mxUtils.bind(this, function () {
      if (Editor.currentTheme == "sketch") {
        ...
        this.statusContainer.style.display = "";
        this.statusContainer.style.display =
          this.statusContainer.clientWidth == 0 ? "none" : "";
      } else {
        this.statusContainer.style.display = "";
      }
    });
    ...
    if (this.isOffline(true)) {
      ...
      if (file != null) {
        if (file.invalidChecksum) {
          status =
            mxResources.get("error") + ": " + mxResources.get("checksum");
        } else if (
          file.sync != null &&
    ...
    if (
      ...
      if (this.actions.get("newLibrary") != null) {
        ...
        mxEvent.addListener(
          div,
          "click",
          this.actions.get("openLibrary").funct,
        );
      } else {
  ...
  EditorUi.prototype.setDarkMode = function (value) {
    if (mxUtils.lightDarkColorSupported) {
      ...
      var node =
        mxUtils.isAncestorNode(document.body, this.container) ||
        this.editor == null
          ? this.container
          : this.editor.graph.container;
  ...
  EditorUi.prototype.installSettings = function () {
    if (Editor.isSettingsEnabled()) {
       ...
       */
      graph.pageFormat =
        graph.defaultPageFormat != null
          ? graph.defaultPageFormat
   ...
   */
  EditorUi.prototype.writeImageToClipboard = function (
    dataUrl,
    w,
    h,
    type,
    ...
    if (ClipboardItem.supports != null && ClipboardItem.supports(type)) {
      data[type] = this.base64ToBlob(
        ...
        this.editor.exportToCanvas(
          mxUtils.bind(this, function (canvas, svgRoot) {
            try {
              ...
              var w = Math.round(
                parseInt(svgRoot.getAttribute("width")) / scale,
              );
              var h = Math.round(
                parseInt(svgRoot.getAttribute("height")) / scale,
              );
  ...
  EditorUi.prototype.pasteXml = function (
    ...
    var targetPoint =
      pt != null
        ? pt
        : Editor.pasteAtMousePointer && graph.isMouseInsertPoint()
    ...
    if (compat || this.isCompatibleString(xml)) {
      ...
      targetPoint = targetPoint != null ? targetPoint : graph.getInsertPoint();
        ...
        if (asHtml) {
          ...
          if (
            (elt.textContent != null &&
              ...
              elt.textContent.substring(elt.textContent.length - 9) ==
                "</mxfile>") ||
            (elt.textContent.substring(0, 13) == "<mxGraphModel" &&
              elt.textContent.substring(elt.textContent.length - 15) ==
                "</mxGraphModel>")
          ) {
  ...
  EditorUi.prototype.pasteFromClipboard = function (pt) {
    ...
    if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
      try {
        if (Editor.enableNativeClipboard) {
          this.readGraphModelFromClipboard(
            mxUtils.bind(this, function (xml, svgBlob, pngBlob, img, data) {
              if (xml != null) {
                graph.getModel().beginUpdate();
                try {
                  this.pasteXml(xml, true, null, null, null, pt);
                } finally {
                  graph.getModel().endUpdate();
                }
              } else {
                var blob =
                  svgBlob != null ? svgBlob : pngBlob != null ? pngBlob : img;
            ...
            if (!containsText && file != null) {
              if (graph.isEditing()) {
                this.importFiles(
                  ...
                  0,
                  this.maxImageSize,
                  function (fileData, mimeType, x, y, w, h) {
                    // Inserts image into current text box
        ...
        if (!mxEvent.isConsumed(evt) && !containsText) {
          if (Editor.enableNativeClipboard) {
            ...
            navigator.clipboard
              .read()
              .then(
                mxUtils.bind(this, function (cpData) {
                  this.readTypeFromClipboardData(
                    cpData,
                    "image/svg+xml",
                    mxUtils.bind(this, function (svg) {
                      if (svg != null) {
    ...
    navigator.clipboard
      ...
      .then(
        mxUtils.bind(this, function (data) {
          try {
            EditorUi.debug(
              "EditorUi.readGraphModelFromClipboard",
              [this],
              "data",
              data,
              "types",
            ...
            this.readGraphModelFromHtmlClipboardData(
              data,
              mxUtils.bind(this, function (html, img) {
                if (html != null) {
                  fn(html);
                } else {
                  this.readGraphModelFromImageClipboardData(
                    data,
                    mxUtils.bind(this, function (xml, svgBlob, pngBlob) {
                      if (xml != null) {
                        fn(xml);
                      } else {
                        this.readTypeFromClipboardData(
   ...
   * Opens the given files in the editor.
   */
  EditorUi.prototype.readGraphModelFromImageClipboardData = function (
    data,
    fn,
  ) {
    this.readTypeFromClipboardData(
      data,
      "image/svg+xml",
      mxUtils.bind(this, function (svgData, svgBlob) {
        var xml = null;
        ...
        if (this.isCompatibleString(xml)) {
          ...
          this.readTypeFromClipboardData(
            data,
            "image/png",
            mxUtils.bind(this, function (pngData, pngBlob) {
              xml = Editor.extractGraphModelFromPng(
                  ...
                  if (type == "image/png") {
                    ...
                    blob
                      ...
                      .then(
                        mxUtils.bind(this, function (result) {
                          EditorUi.debug(
                            "EditorUi.readTypeFromClipboardData",
                            [this],
                            "data",
                            [data],
                            "type",
  ...
  EditorUi.prototype.showPrintDialog = function (title, fn, btnTitle) {
    ...
    this.showDialog(
      new PrintDialog(this, title, fn, btnTitle).container,
      320,
      h,
      true,
      true,
  ...
  EditorUi.prototype.openFileHandle = function (
    ...
    if (name != null && name.length > 0) {
      if (
        (!Editor.useCanvasForExport && /(\.png)$/i.test(name)) ||
        /(\.pdf)$/i.test(name)
      ) {
  ...
  EditorUi.prototype.setGraphEnabled = function (enabled) {
    ...
    if (this.tabContainer != null) {
      this.tabContainer.style.visibility =
        this.diagramContainer.style.visibility;
    }
        ...
        if (urlParams["proto"] == "json") {
          ...
          try {
            if (data == null) {
              ...
              this.executeLayouts(
                this.editor.graph.createLayouts(data.layouts),
              );
              ...
              if (data.viewport != null) {
                ...
                graph.refresh();
                this.diagramContainer.scrollTop -=
                  (ty - graph.view.translate.y) * graph.view.scale;
                this.diagramContainer.scrollLeft -=
                  (tx - graph.view.translate.x) * graph.view.scale;
              ...
              if (data.format == "png" || data.format == "xmlpng") {
                if (
                  ...
                  var xml =
                    data.xml != null ? data.xml : this.getFileData(true);
                  ...
                  if (this.editor.isExportToCanvas()) {
                    ...
                    req.send(
                      mxUtils.bind(this, function (req) {
                        ...
                        if (req.getStatus() >= 200 && req.getStatus() <= 299) {
                          postDataBack(
                            "data:image/png;base64," + req.getText(),
                          );
                        } else {
                          processUri(null);
                        }
                ...
                var graphReady = mxUtils.bind(this, function () {
                  ...
                  if (
                    ...
                    var postResult = mxUtils.bind(this, function (svg) {
                      ...
                      msg.data = data.asText
                        ? svg
                        : Editor.createSvgDataUri(svg);
                      parent.postMessage(JSON.stringify(msg), "*");
                    });
              ...
              if (
                ...
                if (!this.editor.chromeless || this.editor.editable) {
                  ...
                  if (urlParams["noSaveBtn"] != "1") {
                    this.actions.get("save").shortcut =
                      Editor.ctrlKey + "+" + Editor.shiftKey + "+S";
  ...
  EditorUi.prototype.setEmbedTitle = function (filename) {
    ...
    if (Editor.currentTheme == "kennedy" || Editor.currentTheme == "atlas") {
  ...
  EditorUi.prototype.addEmbedButtons = function () {
    this.dependsOnLanguage(
      mxUtils.bind(this, function () {
        var restoreEmbedFilenameSpan =
          this.embedFilenameSpan != null &&
  ...
  EditorUi.prototype.showImportCsvDialog = function () {
    if (this.importCsvDialog == null) {
      this.importCsvDialog = new SimpleTextareaDialog(
        this,
        ...
        try {
          ...
          if (cells.length > 0) {
            ...
            if (layout.charAt(0) == "[") {
              ...
              var orgChartLayoutIsVertexIgnored =
                orgChartLayout.isVertexIgnored;
  ...
  EditorUi.prototype.updateActionStates = function () {
    ...
    this.actions
      .get("explore")
      .setEnabled(active && (ss.edges.length == 1 || ss.vertices.length == 1));
    this.actions
    ...
    this.actions
      .get("tags")
      .setEnabled(this.diagramContainer.style.display != "none");
    this.actions
      .get("layers")
      .setEnabled(this.diagramContainer.style.display != "none");
    this.actions
      ...
      mxUtils.bind(this, function (db) {
        try {
          ...
          for (var i = 0; i < storeName.length; i++) {
            trx.objectStore(storeName[i]).put(
              key != null && key[i] != null
                ? {
                    key: key[i],
                    data: data[i],
 ...
 */
export function CommentsWindow(editorUi, x, y, w, h, saveCallback) {
  ...
  function addComment(comment, parentArr, parent, level, showResolved) {
    ...
    function addReply(initContent, editIt, saveCallback, doResolve, doReopen) {
      ...
      var newReply = editorUi.newComment(
        initContent,
    ...
    ) //Resolve is a top-level action only
    {
      function toggleResolve(evt) {
        function doToggle() {
          ...
          var color =
            "light-dark(" +
            (comment.isResolved ? "ghostWhite" : "white") +
            ", transparent)";
          ...
          for (var i = 0; i < replies.length; i++) {
            ...
            var forOpenActions =
              replies[i].querySelectorAll(".geCommentAction");
 ...
 */
export function HeadlessEditorUi() {
