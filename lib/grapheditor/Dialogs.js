 ...
 */
export function ColorDialog (editorUi, color, apply, cancelFn, defaultColor, defaultColorValue, singleColorMode) {
        ...
        editorUi.prompt(mxResources.get('opacity') + ' (0-100)', opacity, function (newValue) {
            if (!isNaN(newValue) && newValue !== '' &&
                ...
                input.value = mxUtils.rgba2hex(mxUtils.addAlphaToColor(
                    '#' + input.value, newValue / 100, true)).substring(1).toUpperCase();
                darkInput.value = mxUtils.rgba2hex(mxUtils.addAlphaToColor(
                    '#' + darkInput.value, newValue / 100, true)).substring(1).toUpperCase();
        ...
        if (isDefaultColor(elt.value) && cssDefaultColor != null) {
            ...
            if (elt == input && darkSelect.value == 'automatic') {
                if (input.value == mxConstants.NONE) {
                    ...
                    darkInput.value = mxUtils.rgba2hex(mxUtils.getInverseColor('#' + elt.value)).substring(1).toUpperCase();
                }
            }
 ...
 */
export function EditDiagramDialog(editorUi) {
    var div = document.createElement('div');
    div.style.position = 'absolute';
 ...
 */
export function ExportDialog(editorUi) {
    var graph = editorUi.editor.graph;
                        ...
                        if (graph.model.isRoot(cell)) {
                            ...
                            if (page != null) {
                                if (ui.getPageById(value) == null) {
                                    ...
                                    ui.handleError({
                                        message: mxResources.get('alreadyExst',
