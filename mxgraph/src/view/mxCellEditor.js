...
mxCellEditor.prototype.isStopEditingEvent = function (evt) {
  return evt.keyCode == 113 /* F2 */ || (this.graph.isEnterStopsCellEditing() && evt.keyCode == 13 /* Enter */ && !mxEvent.isControlDown(evt) && !mxEvent.isShiftDown(evt));
};
  ...
  if (state == null) {
    ...
    if (!this.autoSize || state.style[mxConstants.STYLE_OVERFLOW] == "fill") {
      ...
      if (
        ...
        (state.text == null || state.text.node == null || state.text.node.getElementsByTagName("text").length == 0)
      ) {
        var dir = mxUtils.getValue(state.style, mxConstants.STYLE_TEXT_DIRECTION, mxConstants.DEFAULT_TEXT_DIRECTION);
...
mxCellEditor.prototype.updateTextAreaStyle = function (state) {
  ...
  if (size == "inherit") {
    size = parentStyle != null ? mxUtils.getValue(parentStyle, mxConstants.STYLE_FONTSIZE, mxConstants.DEFAULT_FONTSIZE) : mxConstants.DEFAULT_FONTSIZE;
  }

  if (family == "inherit") {
    family = parentStyle != null ? mxUtils.getValue(parentStyle, mxConstants.STYLE_FONTFAMILY, mxConstants.DEFAULT_FONTFAMILY) : mxConstants.DEFAULT_FONTFAMILY;
  }
  ...
  if (dir == mxConstants.TEXT_DIRECTION_AUTO && state != null && state.text != null && state.text.dialect != mxConstants.DIALECT_STRICTHTML && !mxUtils.isNode(state.text.value)) {
