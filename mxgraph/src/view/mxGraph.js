 ...
 */
export const mxGraph = (window.mxGraph = function mxGraph(container, model, renderHint, stylesheet, rendering) {
...
mxGraph.prototype.processChange = function (change) {
  ...
  if (this.getDefaultParent() != null && !this.model.contains(this.getDefaultParent())) {
    this.setDefaultParent(null);
  }
};
      ...
      for (var i = 0; i <= count; i++) {
        var pts =
          breaks == this.horizontalPageBreaks
            ? [new mxPoint(Math.round(bounds.x), Math.round(bounds.y + i * bounds.height)), new mxPoint(Math.round(bounds.x + right), Math.round(bounds.y + i * bounds.height))]
            : [new mxPoint(Math.round(bounds.x + i * bounds.width), Math.round(bounds.y)), new mxPoint(Math.round(bounds.x + i * bounds.width), Math.round(bounds.y + bottom))];
    ...
    if (tmp.length > 0) {
      ...
      for (var i = 0; i < cells.length; i++) {
        if (
          ...
          this.getEdgeValidationError(clones[i], this.model.getTerminal(clones[i], true), this.model.getTerminal(clones[i], false)) != null
        ) {
          clones[i] = null;
        } else {
  ...
  if (cell != null && geo != null) {
    ...
    if (geo.alternateBounds == null) {
      ...
      if (this.collapseToPreferredSize) {
        var gridEnabled = mxUtils.getValue(style, mxConstants.STYLE_AUTOSIZE_GRID, this.gridEnabled ? "1" : "0") == "1";
    ...
    try {
      ...
      if (geo != null) {
        ...
        if (fixedWidth) {
          w =
            geo.width -
            2 * parseFloat(mxUtils.getValue(style, mxConstants.STYLE_SPACING, 2)) -
  ...
  if (cell != null) {
    ...
    if (!this.model.isEdge(cell)) {
      ...
      if (value != null && value.length > 0) {
        ...
        var size = mxUtils.getSizeForString(value, fontSize, style[mxConstants.STYLE_FONTFAMILY], textWidth, style[mxConstants.STYLE_FONTSTYLE]);
        ...
        if (geo != null) {
          ...
          if (this.model.isEdge(cells[i])) {
            ...
            if (bbox != null && this.model.isVertex(parent) && mxUtils.indexOf(ancestors != null ? ancestors : cells, parent) >= 0) {
              var tmp = this.getBoundingBoxFromGeometry([parent], false, ancestors != null ? ancestors : cells, includeStrokeWidth);
...
mxGraph.prototype.getSwimlaneDirection = function (style) {
  ...
  return [mxConstants.DIRECTION_NORTH, mxConstants.DIRECTION_EAST, mxConstants.DIRECTION_SOUTH, mxConstants.DIRECTION_WEST][mxUtils.mod(n, 4)];
};
...
mxGraph.prototype.isValidDropTarget = function (cell, cells, evt) {
  return (
    ...
    ((this.isSplitEnabled() && this.isSplitTarget(cell, cells, evt)) ||
      (!this.model.isEdge(cell) && (this.isSwimlane(cell) || (this.model.getChildCount(cell) > 0 && !this.isCellCollapsed(cell)))))
  );
};
      ...
      for (var i = 0; i < childCount; i++) {
        ...
        if (state != null && this.isCellVisible(cell) && (ignoreFn == null || !ignoreFn(state))) {
          ...
          var hit =
            ...
            (intersection == null && (model.isEdge(cell) || model.isVertex(cell)) && box.x >= x && box.y + box.height <= bottom && box.y >= y && box.x + box.width <= right);
...
mxGraph.prototype.selectCells = function (vertices, edges, parent, selectGroups) {
  ...
  var filter = mxUtils.bind(this, function (cell) {
    return (
      ...
      (((selectGroups || this.model.getChildCount(cell) == 0) && this.model.isVertex(cell) && vertices && !this.model.isEdge(this.model.getParent(cell))) ||
...
mxGraph.prototype.isEventSourceIgnored = function (evtName, me) {
  ...
  return (
    ...
    (name == "select" ||
      name == "option" ||
      (name == "input" && source.type != "checkbox" && source.type != "radio" && source.type != "button" && source.type != "submit" && source.type != "file"))
  );
};
...
mxGraph.prototype.fireMouseEvent = function (evtName, me, sender) {
  ...
  if (
    ...
    (this.doubleTapEnabled && mxClient.IS_TOUCH && (mxEvent.isTouchEvent(me.getEvent()) || mxEvent.isPenEvent(me.getEvent())))
  ) {
    ...
    if (evtName == mxEvent.MOUSE_DOWN) {
      ...
      var valid = cell != null || ((mxEvent.isTouchEvent(me.getEvent()) || mxEvent.isPenEvent(me.getEvent())) && (mxClient.IS_GC || mxClient.IS_SF));
