...
import { mxEllipse } from "../shape/mxEllipse.js";
import { mxActor } from "../shape/mxActor.js";
import { mxRectangleShape } from "../shape/mxRectangleShape.js";
import { mxDoubleEllipse } from "../shape/mxDoubleEllipse.js";
import { mxRhombus } from "../shape/mxRhombus.js";
...
mxCellRenderer.prototype.createShape = function (state) {
  ...
  if (state.style != null) {
    ...
    var stencil = mxCellRenderer.defaultShapes[name] == null ? mxStencilRegistry.getStencil(name) : null;
...
mxCellRenderer.prototype.checkPlaceholderStyles = function (state) {
  ...
  if (state.style != null) {
    if (state.style[mxConstants.STYLE_FONTSIZE] == "inherit" || state.style[mxConstants.STYLE_FONTFAMILY] == "inherit") {
      return true;
    }
...
mxCellRenderer.prototype.inheritFontStyle = function (state, field, key) {
  if (state.text != null && state.text[field] == "inherit") {
    var graph = state.view.graph;
    var pstate = graph.getView().getState(graph.model.getParent(state.cell));
...
mxCellRenderer.prototype.resolveColor = function (state, field, key) {
  ...
  if (shape != null) {
    ...
    if (value == "inherit" || value == mxConstants.STYLE_FILLCOLOR || value == mxConstants.STYLE_STROKECOLOR) {
    ...
    if (referenced != null) {
      ...
      if (rstate != null) {
        ...
        if (rshape != null && field != "indicatorColor" && value != mxConstants.STYLE_STROKECOLOR && value != mxConstants.STYLE_FILLCOLOR) {
...
mxCellRenderer.prototype.createTextShape = function (state, value, dialect) {
  ...
  var text = new this.defaultTextShape(
    value,
    new mxRectangle(),
...
mxCellRenderer.prototype.createLabel = function (state, value) {
  ...
  if (state.style[mxConstants.STYLE_FONTSIZE] > 0 || state.style[mxConstants.STYLE_FONTSIZE] == null || state.style[mxConstants.STYLE_FONTSIZE] == "inherit") {
...
mxCellRenderer.prototype.redrawLabel = function (state, forced) {
  ...
  if (
    state.text != null &&
    (state.text.wrap != wrapping || state.text.clipped != clipping || state.text.overflow != overflow || state.text.dialect != dialect || state.text.convertToSvg != convertToSvg)
  ) {
    state.text.destroy();
    state.text = null;
  }
...
mxCellRenderer.prototype.rotateLabelBounds = function (state, bounds, text, margin, disableRotation) {
  ...
  if (
    ...
    (state.style[mxConstants.STYLE_OVERFLOW] != "fill" &&
      ...
      (state.style[mxConstants.STYLE_OVERFLOW] != "block" || state.style[mxConstants.STYLE_BLOCK_SPACING] == "1"))
  ) {
  ...
  if (theta != 0 && state != null && state.view.graph.model.isVertex(state.cell)) {
    ...
    if (bounds.x != cx || bounds.y != cy) {
      ...
      var pt = mxUtils.getRotatedPoint(new mxPoint(bounds.x, bounds.y), Math.cos(rad), Math.sin(rad), new mxPoint(cx, cy));
    ...
    state.overlays.visit(function (id, shape) {
      ...
      if (!state.view.graph.getModel().isEdge(state.cell)) {
        if (state.shape != null && rot != 0) {
          ...
          var point = mxUtils.getRotatedPoint(new mxPoint(cx, cy), cos, sin, new mxPoint(state.getCenterX(), state.getCenterY()));
