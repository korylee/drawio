 ...
 */
export const mxGraphView = (window.mxGraphView = function mxGraphView(graph) {
  this.graph = graph;
...
mxGraphView.prototype.scaleAndTranslate = function (scale, dx, dy) {
  ...
  this.fireEvent(
    new mxEventObject(mxEvent.SCALE_AND_TRANSLATE, "scale", scale, "previousScale", previousScale, "translate", this.translate, "previousTranslate", previousTranslate),
  );
};
...
mxGraphView.prototype.validate = function (cell) {
  ...
  var cell = cell != null ? cell : this.currentRoot != null ? this.currentRoot : this.graph.getModel().getRoot();
...
mxGraphView.prototype.getBoundingBox = function (state, recurse, update) {
  ...
  if (state != null) {
    ...
    if (recurse) {
      ...
      for (var i = 0; i < childCount; i++) {
        var bounds = this.getBoundingBox(this.getState(model.getChildAt(state.cell, i)), recurse, update);
...
mxGraphView.prototype.redrawBackgroundImage = function (backgroundImage, bg) {
  var bounds = new mxRectangle(this.scale * (this.translate.x + bg.x), this.scale * (this.translate.y + bg.y), this.scale * bg.width, this.scale * bg.height);
  ...
  if (backgroundImage.scale != this.scale || !bounds.equals(backgroundImage.bounds)) {
    ...
    if (
      backgroundImage.node != null &&
      backgroundImage.node.nodeName == "g" &&
      (backgroundImage.node.firstChild.nodeName == "image" || backgroundImage.node.firstChild.nodeName == "svg")
    ) {
...
mxGraphView.prototype.updatePoints = function (edge, points, source, target) {
  if (edge != null && edge.absolutePoints != null && edge.absolutePoints.length > 0) {
    var pts = [];
...
mxGraphView.prototype.isLoopStyleEnabled = function (edge, points, source, target) {
  ...
  if (
    ...
    (!mxUtils.getValue(edge.style, mxConstants.STYLE_ORTHOGONAL_LOOP, false) || ((sc == null || sc.point == null) && (tc == null || tc.point == null)))
  ) {
