 ...
 */
export const mxEdgeHandler = window.mxEdgeHandler = function mxEdgeHandler(state) {
...
mxEdgeHandler.prototype.isVirtualBendsEnabled = function (evt) {
  return (
    ...
    (this.state.style[mxConstants.STYLE_EDGE] == null || this.state.style[mxConstants.STYLE_EDGE] == mxConstants.NONE || this.state.style[mxConstants.STYLE_NOEDGESTYLE] == 1) &&
...
mxEdgeHandler.prototype.createMarker = function () {
  ...
  marker.getCell = function (me) {
    ...
    if (
      (this.graph.isSwimlane(cell) && self.currentPoint != null && this.graph.hitsSwimlaneContent(cell, self.currentPoint.x, self.currentPoint.y)) ||
...
mxEdgeHandler.prototype.createVirtualBends = function () {
  ...
  if (this.abspoints != null && this.abspoints.length > 0 && this.graph.isCellBendable(this.state.cell)) {
...
mxEdgeHandler.prototype.isHandleVisible = function (index) {
  return this.edgeStyle != mxEdgeStyle.EntityRelation || index == 0 || index == this.abspoints.length - 1;
};
...
mxEdgeHandler.prototype.getHandleForEvent = function (me) {
  ...
  if (this.state != null) {
    ...
    var hit = !this.allowHandleBoundsCheck ? null : new mxRectangle(me.getGraphX() - tol, me.getGraphY() - tol, tol, tol);
    var minDistSq = null;

    function checkShape(shape) {
      if (shape != null && (me.isSource(shape) || shape.intersectsRectangle(hit))) {
    ...
    if (this.state.text != null && (me.isSource(this.state.text) || checkShape(this.labelShape))) {
  ...
  if (tt > 0 && this.isSnapToTerminalsEvent(me)) {
    ...
    if (pts != null) {
      for (var i = 0; i < pts.length; i++) {
        if ((i > 0 || !this.state.isFloatingTerminalPoint(true)) && (i < pts.length - 1 || !this.state.isFloatingTerminalPoint(false))) {
...
mxEdgeHandler.prototype.getPreviewTerminalState = function (me) {
  ...
  if (constraintHandler.currentFocus != null && constraintHandler.currentConstraint != null) {
    ...
    if (this.marker.highlight != null && this.marker.highlight.state != null && this.marker.highlight.state.cell == constraintHandler.currentFocus.cell) {
  ...
  if (mxEvent.isShiftDown(me.getEvent()) && mxEvent.isAltDown(me.getEvent())) {
    ...
    return (
      ...
      ((mxEvent.isShiftDown(me.getEvent()) && !mxEvent.isAltDown(me.getEvent())) ||
        ...
        (!mxEvent.isShiftDown(me.getEvent()) && mxEvent.isAltDown(me.getEvent()) && me.getState() != null) ||
        this.marker.highlight.isHighlightAt(clientX, clientY) ||
        ((gridX != clientX || gridY != clientY) && me.getState() == null && this.marker.highlight.isHighlightAt(gridX, gridY)))
    );
  }
};
...
mxEdgeHandler.prototype.updatePreviewState = function (edge, point, terminalState, me, outline) {
  ...
  if (this.outlineConnect && this.marker.highlight != null && this.marker.highlight.shape != null) {
    ...
    if (constraintHandler.currentConstraint != null && constraintHandler.currentFocus != null) {
      this.marker.highlight.shape.stroke = outline ? mxConstants.OUTLINE_HIGHLIGHT_COLOR : "transparent";
      this.marker.highlight.shape.strokewidth = mxConstants.OUTLINE_HIGHLIGHT_STROKEWIDTH / s / s;
      this.marker.highlight.repaint();
    } else if (this.marker.hasValidState()) {
      this.marker.highlight.shape.stroke =
        this.graph.isCellConnectable(me.getCell()) && this.marker.getValidState() != me.getState() ? "transparent" : mxConstants.DEFAULT_VALID_COLOR;
...
mxEdgeHandler.prototype.mouseMove = function (sender, me) {
  if (this.index != null && this.marker != null) {
    ...
    if (this.index <= mxEvent.CUSTOM_HANDLE && this.index > mxEvent.VIRTUAL_HANDLE) {
      ...
      if (constraintHandler.currentConstraint != null && constraintHandler.currentFocus != null && constraintHandler.currentPoint != null) {
    ...
    me.consume();
  } else if (!mxEvent.isShiftDown(me.getEvent()) && this.handle != null && this.mouseDownX != null && this.mouseDownY != null) {
    var tol = this.graph.tolerance;

    if (Math.abs(this.mouseDownX - me.getX()) > tol || Math.abs(this.mouseDownY - me.getY()) > tol) {
...
mxEdgeHandler.prototype.mouseUp = function (sender, me) {
  ...
  if (this.index != null && this.marker != null) {
    ...
    if (me.getX() != this.startX || me.getY() != this.startY) {
      var clone = !this.graph.isIgnoreTerminalEvent(me.getEvent()) && this.cloneEnabled && this.graph.isCloneEvent(me.getEvent()) && this.graph.isCellsCloneable();
    ...
    me.consume();
  } else if (this.handle != null && this.bends != null && !mxEvent.isAltDown(me.getEvent()) && (this.handle == 0 || this.handle == this.bends.length - 1)) {
...
mxEdgeHandler.prototype.connect = function (edge, terminal, isSource, isClone, me) {
  ...
  model.beginUpdate();
  try {
    var constraint = this.constraintHandler != null ? this.constraintHandler.currentConstraint : null;
...
mxEdgeHandler.prototype.redrawHandles = function () {
  ...
  if (this.labelShape != null) {
    ...
    this.labelShape.visible = lab != null && lab.length > 0 && this.graph.isCellEditable(this.state.cell) && this.graph.isLabelMovable(cell) && this.isHandlesVisible();
  }
  ...
  if (this.bends != null && this.bends.length > 0) {
    ...
    this.bends[0].node.style.visibility = !this.isHandlesVisible() || !this.isTerminalHandleVisible(true) ? "hidden" : "";
    ...
    this.bends[bn].node.style.visibility = !this.isHandlesVisible() || !this.isTerminalHandleVisible(false) ? "hidden" : "";
  ...
  if (this.customHandles != null) {
    for (var i = 0; i < this.customHandles.length; i++) {
      ...
      this.customHandles[i].shape.node.style.visibility = this.graph.isEditing() || !this.isHandlesVisible() || !this.isCustomHandleVisible(this.customHandles[i]) ? "hidden" : "";
    }
  }
};
...
mxEdgeHandler.prototype.isHandlesVisible = function () {
  return !this.graph.isCellLocked(this.state.cell) && (mxGraphHandler.prototype.maxCells <= 0 || this.graph.getSelectionCount() <= mxGraphHandler.prototype.maxCells);
};
...
mxEdgeHandler.prototype.refresh = function () {
  if (this.state != null) {
    ...
    var geo = this.graph.getCellGeometry(this.state.cell);
    this.edgeStyle = geo != null ? this.graph.view.getEdgeStyle(this.state, geo.points, source, target) : null;
    this.abspoints = this.getSelectionPoints(this.state);
    this.points = [];
    ...
    if (this.isHandlesVisible() && this.edgeStyle != mxEdgeStyle.EntityRelation) {
