 ...
 */
export const mxEdgeSegmentHandler  =window.mxEdgeSegmentHandler = function mxEdgeSegmentHandler(state) {
...
mxEdgeSegmentHandler.prototype.getCurrentPoints = function () {
  ...
  if (pts != null) {
    ...
    if (
      pts.length == 2 ||
      (pts.length == 3 &&
        ((Math.abs(pts[0].x - pts[1].x) < tol && Math.abs(pts[1].x - pts[2].x) < tol) || (Math.abs(pts[0].y - pts[1].y) < tol && Math.abs(pts[1].y - pts[2].y) < tol)))
    ) {
