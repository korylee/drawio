 ...
 */
export const mxSvgCanvas2D = (window.mxSvgCanvas2D = function mxSvgCanvas2D(root, styleEnabled) {
...
mxSvgCanvas2D.prototype.getLightDarkColor = function (color) {
  return mxUtils.getLightDarkColor(color, null, null, this.adaptiveColors == "simple");
};
...
mxSvgCanvas2D.prototype.createGradientId = function (start, end, alpha1, alpha2, direction) {
  ...
  return (prefix + "-gradient-" + start + "-" + end + "-" + dir).replace(/^[^a-z]+|[^\w:.-]+/gi, "_");
};
...
mxSvgCanvas2D.prototype.createFillPatternId = function (type, strokeSize, color) {
  ...
  return ("mx-pattern-" + type + "-" + strokeSize + "-" + color).toLowerCase().replace(/^[^a-z]+|[^\w:.-]+/gi, "_");
};
...
mxSvgCanvas2D.prototype.addNode = function (filled, stroked) {
  ...
  if (node != null) {
    ...
    if (this.strokeTolerance > 0 && (!filled || s.fillColor == null || (!mxShape.forceFilledPointerEvents && !this.pointerEvents && this.originalRoot == null))) {
...
mxSvgCanvas2D.prototype.getShadowFilter = function () {
  ...
  if (s.scale != 1) {
    ...
    if (tok.length > 0) {
      ...
      if (args.length > 3) {
        function arg(index) {
          return Math.round(parseFloat(args[index]) * s.scale * 100) / 100 + "px";
        }

        tmp = tok[0] + "(" + arg(0) + " " + arg(1) + " " + arg(2) + " " + args.slice(3).join(" ") + (tok.length > 2 ? "(" + tok.slice(2).join("(") : "");
      }
    }
  }
...
mxSvgCanvas2D.prototype.updateFill = function () {
  ...
  var pId = s.fillStyle == null || s.fillStyle == "auto" || s.fillStyle == "solid" ? null : this.getFillPattern(s.fillStyle, this.getCurrentStrokeWidth(), fill, s.scale);
...
mxSvgCanvas2D.prototype.updateTextNodes = function (x, y, w, h, align, valign, wrap, overflow, clip, rotation, dir, g) {
  ...
  if (vertical) {
    ...
    alignItems = align == mxConstants.ALIGN_LEFT ? (rl ? "flex-end" : "flex-start") : align == mxConstants.ALIGN_RIGHT ? (rl ? "flex-start" : "flex-end") : "center";
  ...
  var cssBg = this.state.fontBackgroundColor != null ? this.getLightDarkColor(this.state.fontBackgroundColor) : null;
  var cssBorder = this.state.fontBorderColor != null ? this.getLightDarkColor(this.state.fontBorderColor) : null;
  ...
  mxSvgCanvas2D.createCss(
    ...
    clip,
    dir,
    cssBg != null ? cssBg.cssText : null,
    cssBorder != null ? cssBorder.cssText : null,
...
mxSvgCanvas2D.prototype.getTextCss = function () {
  ...
  var css =
    ...
    mxUtils.htmlEntities(s.fontSize) +
    "px; " +
    "font-family: " +
    mxUtils.parseCssFontFamily(s.fontFamily, true) +
    "; color: " +
    this.getLightDarkColor(mxUtils.htmlEntities(s.fontColor)).cssText +
    "; line-height: " +
    mxUtils.htmlEntities(lh) +
    "; pointer-events: " +
    (this.pointerEvents ? mxUtils.htmlEntities(this.pointerEventsValue) : "none") +
    "; ";
    ...
    for (var i = 0; i < elt.childNodes.length && result; i++) {
      ...
      if (child.nodeType == mxConstants.NODETYPE_TEXT) {
        ...
        if (fontSize.slice(-2) == "px") {
          tspan.style.fontSize = (parseFloat(fontSize) * fontScale) / this.state.fontSize + "em";
        }
  ...
  if (this.textEnabled && str != null) {
    ...
    if (this.foEnabled && format == "html") {
      ...
      if (div != null) {
        ...
        if (this.allowConvertHtmlToSvg && this.convertHtmlToSvg(div.firstChild.firstChild, text, offset)) {
          ...
          this.plainText(x + this.state.dx, y + this.state.dy, w, h, "", align, valign, wrap, overflow, clip, rotation, dir, text);
        } else {
...
mxSvgCanvas2D.prototype.addTextBackground = function (node, str, x, y, w, h, align, valign, overflow) {
  ...
  if (document.body != null && (s.fontBackgroundColor != null || s.fontBorderColor != null)) {
    ...
    if (bbox != null) {
      var n = this.createElement("rect");
      var cssBg = s.fontBackgroundColor != null && s.fontBackgroundColor != mxConstants.NONE ? this.getLightDarkColor(s.fontBackgroundColor) : null;
      var cssBorder = s.fontBorderColor != null && s.fontBorderColor != mxConstants.NONE ? this.getLightDarkColor(s.fontBorderColor) : null;
