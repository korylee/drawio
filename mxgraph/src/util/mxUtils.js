...
var mxUtils = {
  ...
  parseCssFontFamily: function (fontFamily, htmlEntities) {
    if (typeof fontFamily === "string") {
      ...
      for (var i = 0; i < tokens.length; i++) {
        ...
        var hasQuotes =
          tokens[i].length > 2 &&
          ((tokens[i].charAt(tokens[i].length - 1) == '"' && tokens[i].charAt(0) == '"') || (tokens[i].charAt(tokens[i].length - 1) == "'" && tokens[i].charAt(0) == "'"));
  ...
  isDarkColor: function (color) {
    ...
    if (rgb != null) {
      return rgb.a > 0.3 && 0.213 * rgb.r + 0.715 * rgb.g + 0.072 * rgb.b < 128;
    }
  ...
  parseLightDarkColor: function (color, defaultDarkColor, useLightColor) {
    ...
    if (dark == null && (useLightColor || mxUtils.isVarColor(light))) {
      dark = light;
    }

    dark = dark != null ? dark : defaultDarkColor != null ? defaultDarkColor : mxUtils.getInverseColor(light);
  ...
  getInverseColor: function (color, percentage, angle) {
    ...
    return "#" + ((1 << 24) | (R << 16) | (G << 8) | B).toString(16).slice(1) + (a < 1 ? ("0" + Math.round(a * 255).toString(16)).slice(-2) : "");
  },
  ...
  addAlphaToColor: function (color, alpha, replaceExisting) {
    if (alpha != null && color != null && color != "transparent") {
      var rgb = mxUtils.parseColor(color);
    ...
    if (color != null && color != mxConstants.NONE) {
      if (typeof color === "string") {
        ...
        if (!mxUtils.lightDarkColorSupported) {
          ...
          result.cssText = result.light == result.dark ? result.light : "light-dark(" + result.light + ", " + result.dark + ")";
        }
      } else {
        result.light = color;
        result.dark = color;
  ...
  intersects: function (a, b, ignoreSize) {
    ...
    if (!ignoreSize && (rw <= 0 || rh <= 0 || tw <= 0 || th <= 0)) {
      return false;
    }
  ...
  rgba2hex: function (color) {
    ...
    return rgb != null
      ? "#" +
          ...
          (rgb.a < 1 ? ("0" + Math.round(rgb.a * 255).toString(16)).slice(-2) : "")
      : color;
  },
